...
let rec f_tr_doc_of_axml (xml : Xml.xml) : tr_doc =
  ...
  | Xml.Element ("cr_doc", [], xml_list) ->
      ...
      let abstract_opt, abstract_tl, nte_nr =
        f_ts_abstract_opt_of_xml_list date_tl
      in
      let doc_main, doc_main_tl, new_nte_nr =
        f_tu_doc_main_of_xml_list nte_nr abstract_tl
      in
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "Expected cr_doc; got: "; string_of_xml_list [ xml ] ]))

(* preamble *)

and f_ts_preamble_opt_of_xml_list (xml_list : Xml.xml list) :
    ts_preamble option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cs_preamble", [], pcdata_list) ->
          (Some (Cs_preamble (f_string_of_pcdata_list pcdata_list)), tl)
      ...
      | Xml.Element ("cs_refs", [], _) ->
          (None, xml_list)
      | xml ->
          raise
            (Error
               (String.concat ""
                  [ "unexcpected element: "; string_of_xml_list [ xml ] ])))

(* title *)

and f_ts_title_opt_of_xml_list (xml_list : Xml.xml list) :
    ts_title option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cs_title", [], pcdata_list) ->
          (Some (Cs_title (f_string_of_pcdata_list pcdata_list)), tl)
      | Xml.Element ("cs_authors", [], _)
      | Xml.Element ("cu_date_auto", [], _)
      | Xml.Element ("cu_date_custom", [], _)
      | Xml.Element ("cs_abstract", [], _)
...
and f_ts_authors_opt_of_xml_list (xml_list : Xml.xml list) :
    ts_authors option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cs_authors", [], xmls) ->
          (Some (Cs_authors (List.map f_ts_author_of_xml xmls)), tl)
      | Xml.Element ("cu_date_auto", [], _)
      | Xml.Element ("cu_date_custom", [], _)
      | Xml.Element ("cs_abstract", [], _)
      | Xml.Element ("cu_doc_main_chs", [], _)
...
and f_ts_author_of_xml (xml : Xml.xml) : ts_author =
  match xml with
  | Xml.Element ("cs_author", [], pcdata_list) ->
      Cs_author (f_string_of_pcdata_list pcdata_list)
  | x ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_author; got: "; string_of_xml_list [ x ] ]))

(* date *)

and f_tu_date_opt_of_xml_list (xml_list : Xml.xml list) :
    tu_date option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_date_auto", [], [ xml ]) ->
          (Some (Cu_date_auto (f_ts_date_auto_of_xml xml)), tl)
      | Xml.Element ("cu_date_custom", [], [ xml ]) ->
          (Some (Cu_date_custom (f_ts_date_custom_of_xml xml)), tl)
      | Xml.Element ("cs_abstract", [], _)
      | Xml.Element ("cu_doc_main_chs", [], _)
      | Xml.Element ("cu_doc_main_secs", [], _)
      | Xml.Element ("cu_doc_main_pars", [], _)
...
and f_ts_date_auto_of_xml (xml : Xml.xml) : ts_date_auto =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_date_auto; got: "; string_of_xml_list [ xml ] ]))

and f_ts_date_custom_of_xml (xml : Xml.xml) : ts_date_custom =
  match xml with
  | Xml.Element ("cs_date_custom", [], pcdata_list) ->
      Cs_date_custom (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_date_custom; got: "; string_of_xml_list [ xml ] ]))

(* abstract *)

and f_ts_abstract_opt_of_xml_list (xml_list : Xml.xml list) :
    ts_abstract option * Xml.xml list * int =
  match xml_list with
  | [] -> (None, xml_list, 0)
  | hd :: tl -> (
      match hd with
...
and f_tu_doc_main_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    tu_doc_main * Xml.xml list * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
...
and f_ts_chs_of_xml (nte_count : int) (xml : Xml.xml) : ts_chs * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_chs; got: "; string_of_xml_list [ xml ] ]))

and f_tr_ch_of_xml (nte_count : int) (xml : Xml.xml) : tr_ch * int =
  match xml with
  | Xml.Element ("cr_ch", [], xml_list) ->
      let tag_or_id_opt, tag_or_id_tl =
        f_ch_sec_par_tag_or_id_opt_of_xml_list xml_list
      in
      let hdr_opt, hdr_tl, nte_nr =
        f_ch_sec_par_hdr_opt_of_xml_list nte_count tag_or_id_tl
      in
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_ch; got: "; string_of_xml_list [ xml ] ]))

and f_tu_secs_pars_or_blks_of_xml_list (nte_count : int)
    (xml_list : Xml.xml list) : tu_secs_pars_or_blks * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
...
and f_ts_secs_of_xml (nte_count : int) (xml : Xml.xml) : ts_secs * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_secs; got: "; string_of_xml_list [ xml ] ]))
...
and f_tr_sec_of_xml (nte_count : int) (xml : Xml.xml) : tr_sec * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_sec; got: "; string_of_xml_list [ xml ] ]))

and f_tu_pars_or_blks_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    tu_pars_or_blks * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
...
and f_ts_pars_of_xml (nte_count : int) (xml : Xml.xml) : ts_pars * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_pars; got: "; string_of_xml_list [ xml ] ]))
...
and f_tu_par_of_xml (nte_count : int) (xml : Xml.xml) : tu_par * int =
  ...
  | Xml.Element ("cu_par_rpt", [], [ x ]) ->
      (Cu_par_rpt (f_ts_par_rpt_of_xml x), nte_count)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_par_std or cu_par_rpt; got: ";
                string_of_xml_list [ xml ];
              ]))
...
and f_ts_par_rpt_of_xml (xml : Xml.xml) : ts_par_rpt =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_par_rpt; got: "; string_of_xml_list [ xml ] ]))
...
and f_tr_par_std_of_xml (nte_count : int) (xml : Xml.xml) : tr_par_std * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_par_std; got: "; string_of_xml_list [ xml ] ]))

and f_par_main_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_blks * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
...
and f_ts_blks_of_xml (nte_count : int) (xml : Xml.xml) : ts_blks * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_blks; got: "; string_of_xml_list [ xml ] ]))

and f_ts_blks_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_blks * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
...
and f_tu_blk_of_xml (nte_count : int) (xml : Xml.xml) : tu_blk * int =
  ...
  | Xml.Element ("cu_blk_vrb", [], [ x ]) ->
      (Cu_blk_vrb (f_ts_blk_vrb_of_xml x), nte_count)
  | Xml.Element ("cu_blk_nte", [], [ x ]) ->
      (Cu_blk_nte (f_tr_blk_nte_of_xml x), nte_count)
  | Xml.Element ("cu_blk_qtn", [], [ x ]) ->
      (Cu_blk_qtn (f_ts_blk_qtn_of_xml x), nte_count)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_blk_txt, cu_blk_blt, cu_blk_itm, cu_blk_dsp, cu_blk_nte, or cu_blk_qtn; got: ";
                string_of_xml_list [ xml ];
              ]))
...
and f_ts_blk_txt_of_xml nte_count (xml : Xml.xml) : ts_blk_txt * int =
  ...
  | Xml.Element ("cs_blk_txt", [], [ x ]) -> (
      match f_ts_txt_lines_of_xml nte_count x with
      | txt_lines, nte_nr -> (Cs_blk_txt txt_lines, nte_nr))
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_blk_txt; got: "; string_of_xml_list [ xml ] ]))

and f_ts_txt_lines_of_xml nte_count (xml : Xml.xml) : ts_txt_lines * int =
  match xml with
  | Xml.Element ("cs_txt_lines", [], xml_list) -> (
      match f_ts_txt_line_list_of_xml_list nte_count xml_list with
      | txt_line_list, nte_nr -> (Cs_txt_lines txt_line_list, nte_nr))
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_txt_lines; got: "; string_of_xml_list [ xml ] ]))

and f_ts_txt_line_list_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_txt_line list * int =
...
and f_ts_txt_line_of_xml (nte_count : int) (xml : Xml.xml) : ts_txt_line * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_txt_line; got: "; string_of_xml_list [ xml ] ]))

and f_ts_txt_units_of_xml (nte_count : int) (xml : Xml.xml) : ts_txt_units * int
    =
  match xml with
  | Xml.Element ("cs_txt_units", [], xml_list) -> (
      let rec aux (xmls : Xml.xml list) (acc : tu_txt_unit list) (nte_nr : int)
          : tu_txt_unit list * int =
        match xmls with
        | [] -> (acc, nte_nr)
        | hd :: tl -> (
            match hd with
            | Xml.Element ("cu_txt_unit_wysiwyg", [], [ xml ]) ->
                aux tl
                  (Cu_txt_unit_wysiwyg (f_ts_txt_unit_wysiwyg_of_xml xml) :: acc)
                  nte_nr
            | Xml.Element ("cu_txt_unit_emph", [], [ xml ]) ->
                aux tl
                  (Cu_txt_unit_emph (f_ts_txt_unit_emph_of_xml xml) :: acc)
                  nte_nr
            | Xml.Element ("cu_txt_unit_c_ref", [], [ xml ]) ->
                aux tl
                  (Cu_txt_unit_c_ref (f_ts_txt_unit_c_ref_of_xml xml) :: acc)
                  nte_nr
            | Xml.Element ("cu_txt_unit_nte_ref", [], [ xml ]) ->
                aux tl
                  (Cu_txt_unit_nte_ref (f_ts_txt_unit_nte_ref_of_xml nte_nr xml)
                  :: acc)
                  (nte_nr + 1)
            | Xml.Element ("cu_txt_unit_nte_inline", [], [ xml ]) ->
                aux tl
                  (Cu_txt_unit_nte_inline
                     (f_ts_txt_unit_nte_inline_of_xml nte_nr xml)
                  :: acc)
                  (nte_nr + 1)
            | _ ->
                raise
                  (Error
                     (String.concat ""
                        [
                          "expected cu_txt_unit_wysiwyg, cu_txt_unit_emph, cu_txt_unit_c_ref, or cu_txt_unit_nte; got: ";
                          string_of_xml_list [ xml ];
                        ])))
      in
      match aux xml_list [] nte_count with
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_txt_units; got: "; string_of_xml_list [ xml ] ]))

and f_tu_txt_unit_of_xml (nte_count : int) (xml : Xml.xml) : tu_txt_unit =
  match xml with
  | Xml.Element ("cu_txt_unit_wysiwyg", [], [ xml ]) ->
      Cu_txt_unit_wysiwyg (f_ts_txt_unit_wysiwyg_of_xml xml)
  | Xml.Element ("cu_txt_unit_emph", [], [ xml ]) ->
      Cu_txt_unit_emph (f_ts_txt_unit_emph_of_xml xml)
  | Xml.Element ("cu_txt_unit_c_ref", [], [ xml ]) ->
      Cu_txt_unit_c_ref (f_ts_txt_unit_c_ref_of_xml xml)
  | Xml.Element ("cu_txt_unit_nte_ref", [], [ xml ]) ->
      Cu_txt_unit_nte_ref (f_ts_txt_unit_nte_ref_of_xml nte_count xml)
  | Xml.Element ("cu_txt_unit_nte_inline", [], [ xml ]) ->
      Cu_txt_unit_nte_inline (f_ts_txt_unit_nte_inline_of_xml nte_count xml)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_txt_unit_wysiwyg, cu_txt_unit_emph, cu_txt_unit_c_ref, cu_txt_unit_nte_ref, or cu_txt_unit_nte_inline; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_txt_unit_wysiwyg_of_xml (xml : Xml.xml) : ts_txt_unit_wysiwyg =
  match xml with
  | Xml.Element ("cs_txt_unit_wysiwyg", [], pcdata_list) ->
      Cs_txt_unit_wysiwyg (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cs_txt_unit_wysiwyg; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_txt_unit_emph_of_xml (xml : Xml.xml) : ts_txt_unit_emph =
  match xml with
  | Xml.Element ("cs_txt_unit_emph", [], pcdata_list) ->
      Cs_txt_unit_emph (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_txt_unit_emph; got: "; string_of_xml_list [ xml ] ]))

and f_ts_txt_unit_c_ref_of_xml (xml : Xml.xml) : ts_txt_unit_c_ref =
  match xml with
  | Xml.Element ("cs_txt_unit_c_ref", [], [ xml ]) ->
      Cs_txt_unit_c_ref (f_ts_c_ref_of_xml xml)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cs_txt_unit_c_ref; got: "; string_of_xml_list [ xml ];
              ]))

and f_ts_txt_unit_nte_ref_of_xml (nte_count : int) (xml : Xml.xml) :
    ts_txt_unit_nte_ref =
  match xml with
  | Xml.Element ("cs_txt_unit_nte_ref", [], [ xml ]) ->
      Cs_txt_unit_nte_ref (f_ts_nte_ref_of_xml nte_count xml)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cs_txt_unit_nte_ref; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_txt_unit_nte_inline_of_xml (nte_count : int) (xml : Xml.xml) :
    ts_txt_unit_nte_inline =
  match xml with
  | Xml.Element ("cs_txt_unit_nte_inline", [], xml_list) ->
      Cs_txt_unit_nte_inline (f_ts_nte_inline_of_xml_list nte_count xml_list)
  | _ ->
      raise
        (Error
           (String.concat ""
...
and f_ts_txt_units_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_txt_units * int =
  match xml_list with
  | hd :: tl -> (
      match hd with
      ...
      | _ ->
          raise
            (Error
               (String.concat ""
                  [
                    "expected cs_txt_units; got: "; string_of_xml_list xml_list;
                  ])))
  | _ ->
      raise
        (Error
...
and f_ts_blk_qtn_of_xml (xml : Xml.xml) : ts_blk_qtn =
  match xml with
  | Xml.Element ("cs_blk_qtn", [], [ x ]) ->
      Cs_blk_qtn (f_ts_qtn_lines_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
...
and f_ts_qtn_lines_of_xml (xml : Xml.xml) : ts_qtn_lines =
  match xml with
  | Xml.Element ("cs_qtn_lines", [], xml_list) ->
      Cs_qtn_lines (f_tu_qtn_line_list_of_xml_list xml_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_qtn_lines; got: "; string_of_xml_list [ xml ] ]))
...
and f_tu_qtn_line_of_xml (xml : Xml.xml) : tu_qtn_line =
  match xml with
  | Xml.Element ("cu_qtn_line_std", [], [ x ]) ->
      Cu_qtn_line_std (f_ts_qtn_line_std_of_xml x)
  | Xml.Element ("cu_qtn_line_br", [], [ x ]) ->
      Cu_qtn_line_br (f_ts_qtn_line_br_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_qtn_line_std or cu_qtn_line_br; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_qtn_line_std_of_xml (xml : Xml.xml) : ts_qtn_line_std =
  match xml with
  | Xml.Element ("cs_qtn_line_std", [], [ x ]) ->
      Cs_qtn_line_std (f_ts_qtn_units_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_qtn_line_std; got: "; string_of_xml_list [ xml ] ]))

and f_ts_qtn_line_br_of_xml (xml : Xml.xml) : ts_qtn_line_br =
  match xml with
  | Xml.Element ("cs_qtn_line_br", [], [ x ]) ->
      Cs_qtn_line_br (f_ts_qtn_units_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_qtn_line_br; got: "; string_of_xml_list [ xml ] ]))

and f_ts_qtn_units_of_xml (xml : Xml.xml) : ts_qtn_units =
  match xml with
  | Xml.Element ("cs_qtn_units", [], xml_list) ->
      Cs_qtn_units (f_tu_qtn_unit_list_of_xml_list xml_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_qtn_units; got: "; string_of_xml_list [ xml ] ]))
...
and f_tu_qtn_unit_of_xml (xml : Xml.xml) : tu_qtn_unit =
  match xml with
  | Xml.Element ("cu_qtn_unit_wysiwyg", [], [ x ]) ->
      Cu_qtn_unit_wysiwyg (f_ts_qtn_unit_wysiwyg_of_xml x)
  | Xml.Element ("cu_qtn_unit_emph", [], [ x ]) ->
      Cu_qtn_unit_emph (f_ts_qtn_unit_emph_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_qtn_unit_wysiwyg or cu_qtn_unit_emph; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_qtn_unit_wysiwyg_of_xml (xml : Xml.xml) : ts_qtn_unit_wysiwyg =
  match xml with
  | Xml.Element ("cs_qtn_unit_wysiwyg", [], pcdata_list) ->
      Cs_qtn_unit_wysiwyg (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cs_qtn_unit_wysiwyg; got: ";
                string_of_xml_list [ xml ];
              ]))

and f_ts_qtn_unit_emph_of_xml (xml : Xml.xml) : ts_qtn_unit_emph =
  match xml with
  | Xml.Element ("cs_qtn_unit_emph", [], pcdata_list) ->
      Cs_qtn_unit_emph (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_qtn_unit_emph; got: "; string_of_xml_list [ xml ] ]))
...
and f_ts_blk_blt_of_xml (nte_count : int) (xml : Xml.xml) : ts_blk_blt * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_blk_blt; got: "; string_of_xml_list [ xml ] ]))
...
and f_tr_blk_itm_of_xml (nte_count : int) (xml : Xml.xml) : tr_blk_itm * int =
  ...
  | Xml.Element ("cr_blk_itm", [], xml_list) ->
      ...
      let tag_or_id_opt, tag_or_id_tl =
        f_itm_tag_or_id_opt_of_xml_list lbl_tl
      in
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_blk_itm; got: "; string_of_xml_list [ xml ] ]))
...
and f_tr_blk_nte_of_xml (xml : Xml.xml) : tr_blk_nte =
  ...
  | Xml.Element ("cr_blk_nte", [], xml_list) ->
      ...
      { fld_blk_nte_id = id; fld_blk_nte_main = blks }
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_blk_nte; got: "; string_of_xml_list [ xml ] ]))
...
and f_ts_blk_dsp_of_xml (nte_count : int) (xml : Xml.xml) : ts_blk_dsp * int =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_blk_dsp; got: "; string_of_xml_list [ xml ] ]))
...
and f_ts_dsp_lines_of_xml (nte_count : int) (xml : Xml.xml) : ts_dsp_lines * int
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_dsp_lines; got: "; string_of_xml_list [ xml ] ]))
...
and f_tr_dsp_line_of_xml (nte_count : int) (xml : Xml.xml) : tr_dsp_line * int =
  ...
  | Xml.Element
      ( "cu_dsp_line_no_lbl",
        [],
        [ Xml.Element ("cs_dsp_line_no_lbl", [], [ x ]) ] ) -> (
      match f_ts_txt_units_of_xml nte_count x with
      | txt_units, nte_nr ->
          ( {
  ...
  | Xml.Element
      ( "cu_dsp_line_lbld",
        [],
        [ Xml.Element ("cr_dsp_line_lbld", [], xml_list) ] ) ->
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_dsp_line_no_lbl or cu_dsp_line_lbld; got: ";
                string_of_xml_list [ xml ];
              ]))
...
and f_ts_blk_vrb_of_xml (xml : Xml.xml) : ts_blk_vrb =
  match xml with
  | Xml.Element ("cs_blk_vrb", [], [ x ]) ->
      Cs_blk_vrb (f_ts_vrb_lines_of_xml x)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_blk_vrb; got: "; string_of_xml_list [ xml ] ]))

and f_ts_vrb_lines_of_xml (xml : Xml.xml) : ts_vrb_lines =
  match xml with
  | Xml.Element ("cs_vrb_lines", [], xml_list) ->
      Cs_vrb_lines (List.map f_ts_vrb_line_of_xml xml_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_vrb_lines; got: "; string_of_xml_list [ xml ] ]))

and f_ts_vrb_line_of_xml (xml : Xml.xml) : ts_vrb_line =
  match xml with
  | Xml.Element ("cs_vrb_line", [], xml_list) ->
      Cs_vrb_line (f_string_of_pcdata_list xml_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_vrb_line; got: "; string_of_xml_list [ xml ] ]))

(* hdr *)

and f_ts_hdr_opt_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_hdr option * int =
  match xml_list with
  | [] -> (None, nte_count)
  | hd :: tl -> (
      ...
      | Xml.Element ("cs_hdr", [], [ xml ]) -> (
          match f_ts_txt_lines_of_xml nte_count xml with
          | txt_lines, nte_nr -> (Some (Cs_hdr txt_lines), nte_nr))
      | _ -> f_ts_hdr_opt_of_xml_list nte_count tl)

and f_ch_sec_par_hdr_opt_of_xml_list (nte_count : int) (xml_list : Xml.xml list)
    : ts_hdr option * Xml.xml list * int =
  match xml_list with
  ...
  | hd :: tl -> (
      ...
      | Xml.Element ("cs_hdr", [], [ xml ]) -> (
          match f_ts_txt_lines_of_xml nte_count xml with
          | txt_lines, nte_nr -> (Some (Cs_hdr txt_lines), tl, nte_nr))
      | _ -> (None, xml_list, nte_count))

(* tag_or_id *)

and f_ch_sec_par_tag_or_id_opt_of_xml_list (xml_list : Xml.xml list) :
    tu_tag_or_id option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_tag_or_id_tag", [], [ xml ]) ->
          (Some (Cu_tag_or_id_tag (f_ts_tag_of_xml xml)), tl)
      | Xml.Element ("cu_tag_or_id_id", [], [ xml ]) ->
          (Some (Cu_tag_or_id_id (f_tr_id_of_xml xml)), tl)
      | _ -> (None, xml_list))

and f_tu_tag_or_id_opt_of_xml_list (xml_list : Xml.xml list) :
    tu_tag_or_id option =
  match xml_list with
  | [] -> None
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_tag_or_id_tag", [], [ xml ]) ->
          Some (Cu_tag_or_id_tag (f_ts_tag_of_xml xml))
      | Xml.Element ("cu_tag_or_id_id", [], [ xml ]) ->
          Some (Cu_tag_or_id_id (f_tr_id_of_xml xml))
      | _ -> f_tu_tag_or_id_opt_of_xml_list tl)

and f_itm_tag_or_id_opt_of_xml_list (xml_list : Xml.xml list) :
    tu_tag_or_id option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_tag_or_id_id", _, _) ->
          (f_tu_tag_or_id_opt_of_xml_list [ hd ], tl)
      | Xml.Element ("cu_tag_or_id_tag", _, _) ->
          (f_tu_tag_or_id_opt_of_xml_list [ hd ], tl)
      ...
      | xml ->
          raise
            (Error
               (String.concat ""
                  [
                    "expected cu_tag_or_id_id or cu_tag_or_id_tag; got: ";
                    string_of_xml_list [ xml ];
                  ])))
...
and f_dsp_id_opt_of_xml_list (xml_list : Xml.xml list) :
    tr_id option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
...
and f_tr_id_of_xml (xml : Xml.xml) : tr_id =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cr_id; got: "; string_of_xml_list [ xml ] ]))

(* tag *)

and f_ts_tag_of_xml (xml : Xml.xml) : ts_tag =
  match xml with
  | Xml.Element ("cs_tag", [], pcdata_list) ->
      Cs_tag (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_tag; got: "; string_of_xml_list [ xml ] ]))

(* name *)

and f_ts_name_of_xml (xml : Xml.xml) : ts_name =
  match xml with
  | Xml.Element ("cs_name", [], pcdata_list) ->
      Cs_name (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_name; got: "; string_of_xml_list [ xml ] ]))
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [
                "expected cu_scope_gbl, cu_scope_ch, cu_scope_sec, cu_scope_app, or cu_scope_par; got: ";
                string_of_xml_list [ xml ];
              ]))
...
and f_ts_c_ref_of_xml (xml : Xml.xml) : ts_c_ref =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_c_ref; got: "; string_of_xml_list [ xml ] ]))
...
and f_ts_nte_ref_of_xml (nte_count : int) (xml : Xml.xml) : ts_nte_ref =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_nte_ref; got: "; string_of_xml_list [ xml ] ]))

(* nte_inline *)

and f_ts_nte_inline_of_xml_list (nte_count : int) (xml_list : Xml.xml list) :
    ts_nte_inline =
...
and f_itm_lbl_of_xml_list (xml_list : Xml.xml list) : tu_lbl * Xml.xml list =
  ...
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_lbl_auto", [], [ xml ]) ->
          (Cu_lbl_auto (f_ts_lbl_auto_of_xml xml), tl)
      | Xml.Element ("cu_lbl_custom", [], [ xml ]) ->
          (Cu_lbl_custom (f_ts_lbl_custom_of_xml xml), tl)
      | xml ->
          raise
            (Error
               (String.concat ""
                  [
                    "expected cu_lbl_auto or cu_lbl_custom; got: ";
                    string_of_xml_list [ xml ];
                  ])))
  | [] ->
      raise
...
and f_ts_lbl_auto_of_xml (xml : Xml.xml) : ts_lbl_auto =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_lbl_auto; got: "; string_of_xml_list [ xml ] ]))

and f_ts_lbl_custom_of_xml (xml : Xml.xml) : ts_lbl_custom =
  match xml with
  | Xml.Element ("cs_lbl_custom", [], pcdata_list) ->
      Cs_lbl_custom (f_string_of_pcdata_list pcdata_list)
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected cs_lbl_custom; got: "; string_of_xml_list [ xml ] ]))

and f_dsp_lbl_opt_of_xml_list (xml_list : Xml.xml list) :
    tu_lbl option * Xml.xml list =
  match xml_list with
  | [] -> (None, xml_list)
  | hd :: tl -> (
      match hd with
      | Xml.Element ("cu_lbl_auto", [], [ xml ]) ->
          (Some (Cu_lbl_auto (f_ts_lbl_auto_of_xml xml)), tl)
      | Xml.Element ("cu_lbl_custom", [], [ xml ]) ->
          (Some (Cu_lbl_custom (f_ts_lbl_custom_of_xml xml)), tl)
      | _ -> (None, xml_list))
...
and f_string_of_pcdata (pcdata : Xml.xml) : string =
  ...
  | _ ->
      raise
        (Error
           (String.concat ""
              [ "expected pcdata; got: "; string_of_xml_list [ pcdata ] ]))
