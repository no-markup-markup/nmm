...
let dsp_custom_tab =
  [%sedlex.regexp? "(", Plus (Compl non_custom_chars), ")", tab]
...
let sec_tag_or_id =
  [%sedlex.regexp? ("SEC" | "APP"), Opt (":", name, Opt (":", ("GBL" | "CH")))]

let par_tag_or_id =
  [%sedlex.regexp?
    ( ("PAR" | tag_shared),
      Opt (":", name, Opt (":", ("GBL" | "CH" | "SEC" | "APP"))) )]

let itm_id =
  [%sedlex.regexp? ("ITM" | "BIB" | tag_shared), ":", name, Opt (":", scope)]
...
let par_id =
  [%sedlex.regexp?
    ("PAR" | tag_shared), ":", name, Opt (":", ("GBL" | "CH" | "SEC" | "APP"))]
...
let section_spaces_tag_or_id_nl =
  [%sedlex.regexp? section, spaces, sec_tag_or_id, nl]

let pilcrow_nl = [%sedlex.regexp? pilcrow, nl]

let pilcrow_spaces_tag_or_id_nl =
  [%sedlex.regexp? pilcrow, spaces, par_tag_or_id, nl]
...
let get_esc_char (s : string) : string = String.sub s 1 (String.length s - 1)
let get_label (s : string) : string = String.sub s 1 (String.length s - 3)
...
let lexeme (lexbuf : Sedlexing.lexbuf) : string = Sedlexing.Utf8.lexeme lexbuf
...
let set_end_of_file_and_return_token (tkn : Nmm_parser.token) : Nmm_parser.token
    =
  let _ : unit = lexer_env.end_of_file <- true in
  tkn
...
let rec token (lexbuf : Sedlexing.lexbuf) : Nmm_parser.token =
  ...
  | false, false, false, false -> (
      ...
      | section_spaces_tag_or_id_nl ->
          SECTION_SPACES_TAG_OR_ID_NL (get_tag_or_id (lexeme lexbuf))
      | pilcrow_nl -> PILCROW_NL
      | pilcrow_spaces_tag_or_id_nl ->
          PILCROW_SPACES_TAG_OR_ID_NL (get_tag_or_id (lexeme lexbuf))
      | pilcrow_spaces_rpt_spaces_id_nl ->
          PILCROW_SPACES_RPT_SPACES_ID_NL (get_id (lexeme lexbuf))
      ...
      | dsp_custom_tab ->
          set_display_and_return_token
            (DSP_CUSTOM_TAB (get_label (lexeme lexbuf)))
      ...
      | nl ->
          let _ : unit = skip_newlines lexbuf in
          NL
      | nl_tab -> NL_TAB
      ...
      | eof ->
          if lexer_env.end_of_file then EOF
          else set_end_of_file_and_return_token NL
      | _ ->
          raise
            (ERROR
               ("unexpected string on line " ^ line_of_lexbuf lexbuf ^ ": \""
              ^ lexeme lexbuf ^ "\"")))
  | true, _, _, _ -> (
      match%sedlex lexbuf with
      ...
      | tab_tab_tab_end_vrb ->
          reset_verbatim_and_return_token TAB_TAB_TAB_END_VRB
