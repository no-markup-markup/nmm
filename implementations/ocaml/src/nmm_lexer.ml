...
let scope = [%sedlex.regexp? "CH" | "SEC" | "APP" | "PAR"]
...
let sec_tag_or_id = [%sedlex.regexp? "SEC", Opt (":", name, Opt (":CH"))]
let par_tag_or_id = [%sedlex.regexp? "PAR", Opt (":", name, Opt (":CH" | ":SEC"))]
let itm_id = [%sedlex.regexp? "ITM", ":", name, Opt (":CH" | ":SEC" | ":PAR")]
let dsp_id = [%sedlex.regexp? "DSP", ":", name, Opt (":CH" | ":SEC" | ":PAR")]
