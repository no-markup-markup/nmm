...
let utf_8_segments (boundary : Uuseg.boundary) (s : string) : string list =
  ...
  in
  let rec add (buf : Buffer.t) (acc : string list) (segmenter : Uuseg.t)
      (v : [ `Await | `End | `Uchar of Uchar.t ]) : string list =
    ...
    | `Await | `End -> acc
  in
  let rec loop (buf : Buffer.t) (acc : string list) (s : string) (i : int)
      (max : int) (segmenter : Uuseg.t) : string list =
    if i > max then flush_segment buf (add buf acc segmenter `End)
    else
      let dec : Uchar.utf_decode = String.get_utf_8_uchar s i in
      let acc : string list =
        add buf acc segmenter (`Uchar (Uchar.utf_decode_uchar dec))
      in
...
let utf_8_length (s : string) : int = List.length (utf_8_grapheme_clusters s)
...
let line_break (line_width : int) (s : string) : string list =
  ...
  let rec aux (remaining_graphemes : string list)
      (acc_line : (string * string * int) option)
      (acc_word : (string * int) option) (acc : string list) : string list =
    ...
    | hd :: tl -> (
        ...
        | Some (l, s, n), None -> (
            ...
            | true -> (
                match hd with
                | " " | " \u{0332}" ->
                    aux tl (Some (l ^ s, hd, n + 1)) acc_word acc
                | _ -> aux tl acc_line (Some (hd, 1)) acc)
            | false -> (
                match hd with
        ...
        | Some (l, s, i), Some (w, j) -> (
            ...
            | true -> (
                match hd with
                | " " | " \u{0332}" ->
                    aux tl (Some (l ^ s ^ w, hd, i + j + 1)) None acc
...
let lines_of_ts_title (doc_settings : t_doc_settings) (title : ts_title) :
    string list =
  match title with
  | Cs_title (s : string) ->
      let indent : string = String.make doc_settings.title_indent ' ' in
      let overline : string =
        make_string (doc_settings.doc_width - doc_settings.title_indent) "═"
      in
...
let lines_of_ts_title_opt (doc_settings : t_doc_settings)
    (title_opt : ts_title option) : string list =
...
let lines_of_ts_author (doc_settings : t_doc_settings) (author : ts_author) :
    string list =
  match author with
  | Cs_author (s : string) ->
      List.concat
...
let lines_of_ts_authors (doc_settings : t_doc_settings) (authors : ts_authors) :
    string list =
  match authors with
  | Cs_authors (author_list : ts_author list) ->
      List.concat
        [ List.concat (List.map (lines_of_ts_author doc_settings) author_list) ]

let lines_of_ts_authors_opt (doc_settings : t_doc_settings)
    (authors_opt : ts_authors option) : string list =
...
let lines_of_ts_date_custom (doc_settings : t_doc_settings)
    (date : ts_date_custom) : string list =
  match date with
...
let lines_of_ts_date_auto (doc_settings : t_doc_settings) (date : ts_date_auto)
  ...
  | Some time ->
      ...
      let date_string : string =
        String.concat "-"
          [ format time.year; format time.month; format time.day ]
      in
      let time_string : string =
        String.concat ":" [ format time.hour; format time.minute ]
      in
      let s : string =
        String.concat " "
...
let lines_of_tu_date_opt (doc_settings : t_doc_settings)
    (date_opt : tu_date option) : string list =
  match date_opt with
  | None -> []
...
let underline (s : string) : string =
  ...
  let map (el : string) : string =
    match el with "g" | "j" | "p" | "q" | "y" -> el | _ -> el ^ "\u{0332}"
  in
  String.concat "" (List.map map lst)

let emph (a : string) : string = underline a

let string_of_tu_txt_unit (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : tu_txt_unit) : string =
  match a with
  | Cu_txt_unit_wysiwyg (Cs_txt_unit_wysiwyg (b : string)) -> b
  | Cu_txt_unit_emph (Cs_txt_unit_emph (b : string)) -> emph b
  | Cu_txt_unit_c_ref (Cs_txt_unit_c_ref (b : ts_c_ref)) ->
      string_of_ts_c_ref doc_settings cref_table path b
  | Cu_txt_unit_nte_ref (Cs_txt_unit_nte_ref (b : ts_nte_ref)) ->
      string_of_ts_nte_ref doc_settings nte_table path b
  | Cu_txt_unit_nte_inline (Cs_txt_unit_nte_inline (b : ts_nte_inline)) ->
      string_of_ts_nte_inline doc_settings nte_table path b

let string_of_ts_txt_units (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : ts_txt_units) : string =
  match a with
  | Cs_txt_units (b : tu_txt_unit list) ->
      String.concat ""
        (List.map
           (string_of_tu_txt_unit doc_settings cref_table nte_table path)
           b)

let lines_of_ts_txt_units (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
  ...
  lines_of_string_function
    ...
    (string_of_ts_txt_units doc_settings cref_table nte_table path a)

let string_of_ts_txt_lines (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (txt_lines : ts_txt_lines) : string =
  string_of_ts_txt_units doc_settings cref_table nte_table path
    (Cs_txt_units (Common_utils.txt_units_of_txt_lines txt_lines))

(* abstract *)

let lines_of_abstract_hdr (doc_settings : t_doc_settings)
    (doc_class : t_doc_class) : string list =
...
let lines_of_ts_hdr (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (hdr : ts_hdr) : string list =
  match hdr with
  | Cs_hdr (txt_lines : ts_txt_lines) -> (
      match path with
      | path_hd :: path_tl -> (
          let indent : string = String.make doc_settings.left_margin ' ' in
          let hdr_string : string =
            string_of_ts_txt_lines doc_settings cref_table nte_table path
              txt_lines
          in
          ...
          | SEC_NODE _ | APP_NODE _ -> (
              let underline =
                make_string
                  (Int.min (utf_8_length hdr_string)
                     (doc_settings.doc_width - doc_settings.left_margin))
                  "─"
              in
              ...
              | hd :: tl ->
                  List.concat
                    [
                      [ insert_label doc_settings path hd ];
                      tl;
                      [ indent ^ underline; "" ];
                    ]
              | [] -> [])
          | CH_NODE _ ->
              ...
              in
              List.concat
                [ [ indent ^ label ]; hdr_lines; [ indent ^ underline; "" ] ]
          | _ -> [])
      | [] -> [])

let lines_of_ts_hdr_opt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (hdr_opt : ts_hdr option) : string list =
  match hdr_opt with
  | Some (hdr : ts_hdr) ->
      lines_of_ts_hdr doc_settings cref_table nte_table path hdr
  | None -> (
      match path with
      | hd :: tl -> (
          match hd with
...
let special_tag (doc_settings : t_doc_settings) (a : tu_tag_or_id option) :
    tu_txt_unit option =
  match a with
  | None -> None
  | Some (b : tu_tag_or_id) -> (
      ...
      | Cu_tag_or_id_id { fld_id_tag = (tag : ts_tag); fld_id_name = _ } -> (
          ...
          | Some (lbl, _) ->
              Some (Cu_txt_unit_wysiwyg (Cs_txt_unit_wysiwyg lbl))
          | None -> None))

let copy_hdr_to_main (doc_settings : t_doc_settings) (par : tr_par_std) :
    tr_par_std =
  ...
  match
    ( special_tag doc_settings par.fld_par_tag_or_id,
      par.fld_par_hdr,
      par.fld_par_main )
  with
  | ( Some (s : tu_txt_unit),
                     ...
                     (Cs_txt_line
                        (Cs_txt_units
                           (List.concat
                              [
                                [ s; space; lpar ];
                                Common_utils.txt_units_of_txt_lines h;
                                [ rpar; space ];
                              ]))
                     :: txt_lines)))
            :: tl);
                  ...
                  (Cs_txt_lines
                     (Cs_txt_line
                        (Cs_txt_units
                           (List.concat
                              [
                                Common_utils.txt_units_of_txt_lines h; [ space ];
                              ]))
                     :: txt_lines)))
            :: tl);
      }
        ...
        fld_par_main =
          Cs_blks
            (Cu_blk_txt
               (Cs_blk_txt
                  (Cs_txt_lines
                     (Cs_txt_line (Cs_txt_units [ s; space ]) :: txt_lines)))
            :: tl);
      }
  | ( Some (s : tu_txt_unit),
      Some (Cs_hdr (h : ts_txt_lines)),
  ...
  | Some (s : tu_txt_unit), None, Cs_blks (blks : tu_blk list) ->
      {
        fld_par_tag_or_id = par.fld_par_tag_or_id;
        fld_par_hdr = par.fld_par_hdr;
        fld_par_main =
          Cs_blks
            (Cu_blk_txt
               (Cs_blk_txt (Cs_txt_lines [ Cs_txt_line (Cs_txt_units [ s ]) ]))
           :: blks);
      }

(* refs_hdr *)

let lines_of_refs_hdr (doc_settings : t_doc_settings) (doc_class : t_doc_class)
    : string list =
  match doc_settings.refs_hdr with
  | None -> []
  | Some (hdr, _) ->
      let underline_symbol : string =
        match doc_class with DOC_CHS -> "═" | _ -> "─"
      in
      let indent : string = String.make doc_settings.refs_indent ' ' in
      let underline =
        make_string
          (Int.min (utf_8_length hdr)
             (doc_settings.doc_width - doc_settings.refs_indent))
          underline_symbol
      in
      let hdr_lines : string list =
...
let lines_of_endnotes_hdr (doc_settings : t_doc_settings) (path : t_path) :
    string list =
...
let lines_of_ts_blk_txt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (blk_txt : ts_blk_txt) : string list =
  match blk_txt with
  | Cs_blk_txt (txt_lines : ts_txt_lines) ->
      lines_of_ts_txt_units doc_settings cref_table nte_table path
        (Cs_txt_units (Common_utils.txt_units_of_txt_lines txt_lines))

(* dsp_line *)

let lines_of_tr_dsp_line (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : tr_dsp_line) : string list =
  match
    ( a.fld_dsp_line_lbl,
      lines_of_ts_txt_units doc_settings cref_table nte_table path
        a.fld_dsp_line_units )
  with
...
let string_of_ts_qtn_unit_emph (qtn_unit_emph : ts_qtn_unit_emph) : string =
  match qtn_unit_emph with Cs_qtn_unit_emph s -> emph s

let string_of_ts_qtn_unit_wysiwyg (qtn_unit_wysiwyg : ts_qtn_unit_wysiwyg) :
    string =
  match qtn_unit_wysiwyg with Cs_qtn_unit_wysiwyg s -> s

let string_of_tu_qtn_unit (qtn_unit : tu_qtn_unit) : string =
  match qtn_unit with
  | Cu_qtn_unit_wysiwyg qtn_unit_wysiwyg ->
      string_of_ts_qtn_unit_wysiwyg qtn_unit_wysiwyg
...
let lines_of_tu_qtn_line_list (doc_settings : t_doc_settings) (path : t_path)
    (qtn_line_list : tu_qtn_line list) : string list =
  let rec aux (lst : tu_qtn_line list) (acc : string list) =
    match lst with
    | [] -> List.rev acc
    | lst_hd :: lst_tl -> (
        match (lst_hd, acc) with
        | Cu_qtn_line_std qtn_line_std, acc_hd :: acc_tl ->
            aux lst_tl
              ((acc_hd ^ " " ^ string_of_qtn_line_std qtn_line_std) :: acc_tl)
        | Cu_qtn_line_std qtn_line_std, _ ->
            aux lst_tl (string_of_qtn_line_std qtn_line_std :: acc)
        | Cu_qtn_line_br qtn_line_br, _ ->
            aux lst_tl (string_of_qtn_line_br qtn_line_br :: acc))
  in
...
let lines_of_ts_qtn_lines (doc_settings : t_doc_settings) (path : t_path)
  ...
  | Cs_qtn_lines qtn_line_list ->
      lines_of_tu_qtn_line_list doc_settings path qtn_line_list

let lines_of_ts_blk_qtn (doc_settings : t_doc_settings) (path : t_path)
    (blk_qtn : ts_blk_qtn) : string list =
  match blk_qtn with
  | Cs_blk_qtn qtn_lines -> lines_of_ts_qtn_lines doc_settings path qtn_lines

(* blk_vrb *)

let line_of_vrb_line (doc_settings : t_doc_settings) (path : t_path)
    (vrb_line : ts_vrb_line) : string =
  match vrb_line with
  | Cs_vrb_line (line : string) -> (
      let indent : string =
        String.make (indent_of_path doc_settings path) ' '
      in
      match line with "" -> "" | _ -> String.concat "" [ indent; line ])

let lines_of_ts_vrb_lines (doc_settings : t_doc_settings) (path : t_path)
    (vrb_lines : ts_vrb_lines) : string list =
  match vrb_lines with
  | Cs_vrb_lines (vrb_line_list : ts_vrb_line list) ->
      List.map (line_of_vrb_line doc_settings path) vrb_line_list

let lines_of_ts_blk_vrb (doc_settings : t_doc_settings) (path : t_path)
    (blk_vrb : ts_blk_vrb) : string list =
  match blk_vrb with
  | Cs_blk_vrb (vrb_lines : ts_vrb_lines) ->
      lines_of_ts_vrb_lines doc_settings path vrb_lines
