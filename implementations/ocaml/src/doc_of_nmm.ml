...
let string_of_token (t : Nmm_parser.token) : string =
  ...
  | SECTION_SPACES_TAG_OR_ID_NL s ->
      "SECTION_SPACES_TAG_OR_ID_NL " ^ "\"" ^ s ^ "\""
  | PILCROW_SPACES_TAG_OR_ID_NL s ->
      "PILCROW_SPACES_TAG_OR_ID_NL " ^ "\"" ^ s ^ "\""
  | PILCROW_SPACES_RPT_SPACES_ID_NL s ->
      "PILCROW_SPACES_RPT_SPACES_ID_NL " ^ "\"" ^ s ^ "\""
  | TXT s -> "TXT " ^ "\"" ^ s ^ "\""
...
let sedlexer (print_tokens : bool) (b : Sedlexing.lexbuf) :
    Nmm_parser.token * Lexing.position * Lexing.position =
  ...
  | true ->
      let _ =
        IO.print_to_stderr
          ("Line " ^ Nmm_lexer.line_of_lexbuf b ^ ": " ^ string_of_token token)
      in
...
let rec doc_of_nmm_file (print_tokens : bool) (filename : string) :
    Doc_types.tr_doc =
...
let doc_of_nmm_file_with_tagger
    (tagger : Doc_types.tr_blk_itm -> Doc_types.tr_blk_itm) (path : string) :
    Doc_types.tr_doc =
  let _ : unit = Nmm_parser.blk_itm_tagger_ref.contents <- tagger in
  doc_of_nmm_file false path

let doc_of_nmm_stdin_with_tagger
    (tagger : Doc_types.tr_blk_itm -> Doc_types.tr_blk_itm) : Doc_types.tr_doc =
  let _ : unit = Nmm_parser.blk_itm_tagger_ref.contents <- tagger in
  doc_of_nmm_stdin false

let doc_of_nmm_string_with_tagger
    (tagger : Doc_types.tr_blk_itm -> Doc_types.tr_blk_itm) (s : string) :
    Doc_types.tr_doc =
  let _ : unit = Nmm_parser.blk_itm_tagger_ref.contents <- tagger in
