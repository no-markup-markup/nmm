...
let nte_numbering_default (n : int) : string = superscript_string_of_int (n + 1)
...
let doc_settings_of_ts_blks (doc_settings : t_doc_settings) (lvl : int)
    (blks : Doc_types.ts_blks) : t_doc_settings =
  if not doc_settings.allow_custom_numbering then doc_settings
  else
              ...
              | Cu_lbl_custom (Cs_lbl_custom s) -> (
                  ...
                  | "1" | "a" | "i" ->
                      let inc : int =
                        match
                          ( s,
                            doc_settings.auto_numbering 1 0,
                            doc_settings.auto_numbering 2 0 )
                        with
                        | "a", "a", _ -> 1
                        | "a", _, "a" -> 2
                        | "1", "1", _ -> 1
...
let prefix_value_of_string (v : string) : (string * string) option =
  ...
  | [ s ] -> ( match s with "" -> None | _ -> Some (s, s))
  | _ -> None

let endnotes_hdr_of_string (v : string) : string option =
  match v with "" -> None | s -> Some s
  ...
  with _ ->
    let _ : unit =
      IO.print_warning
        (String.concat ""
           [
             "WARNING: invalid left_margin value: ";
             v;
             "\n";
             "using default value.";
           ])
  ...
  with _ ->
    let _ : unit =
      IO.print_warning
        (String.concat ""
           [
             "WARNING: invalid title_indent value: ";
             v;
             "\n";
             "using default value.";
           ])
...
let path_to_ch_node (path : t_path) : t_path =
  let rec aux (rev_path : t_path) (acc : t_path) : t_path =
    ...
    | [] -> acc
    | hd :: tl -> (
        match hd with CH_NODE _ -> hd :: acc | _ -> aux tl (hd :: acc))
  in
  aux (List.rev path) []
...
let path_to_sec_node (path : t_path) : t_path =
  let rec aux (rev_path : t_path) (acc : t_path) : t_path =
    ...
    | [] -> acc
    | hd :: tl -> (
        match hd with SEC_NODE _ -> hd :: acc | _ -> aux tl (hd :: acc))
  in
  aux (List.rev path) []
...
let path_to_app_node (path : t_path) : t_path =
  let rec aux (rev_path : t_path) (acc : t_path) : t_path =
    ...
    | [] -> acc
    | hd :: tl -> (
        match hd with APP_NODE _ -> hd :: acc | _ -> aux tl (hd :: acc))
  in
  aux (List.rev path) []
...
let path_to_par_node (path : t_path) : t_path =
  let rec aux (rev_path : t_path) (acc : t_path) : t_path =
    ...
    | [] -> acc
    | hd :: tl -> (
        match hd with PAR_NODE _ -> hd :: acc | _ -> aux tl (hd :: acc))
  in
  aux (List.rev path) []
...
let c_ref_loc_is_within_scope_of_id (c_ref_loc : t_path)
    (scope_opt : tu_scope option) (id_loc : t_path) : bool =
...
let ids_match (c_ref_id : Doc_types.tr_id) (c_ref_loc : t_path)
    (id : Doc_types.tr_id) (id_loc : t_path) : bool =
  if c_ref_id = id then
    c_ref_loc_is_within_scope_of_id c_ref_loc id.fld_id_scope id_loc
  else false

let reference_of_ts_c_ref (cref_table : t_cref_table) (c_ref_path : t_path)
    (c_ref : Doc_types.ts_c_ref) :
    (Doc_types.tr_id * t_path * t_cref_element) option =
  match c_ref with
  | Cs_c_ref c_ref_id ->
      let rec aux (cref_table : t_cref_table) :
          (Doc_types.tr_id * t_path * t_cref_element) option =
...
let string_of_node_opt (doc_settings : t_doc_settings) (tail : t_path)
    (head : t_node) : string option =
  match head with
  | CH_NODE (n : int) | SEC_NODE (n : int) -> Some (string_of_int (n + 1))
...
let string_of_path_opt (doc_settings : t_doc_settings) (full_path : t_path)
    (path : t_path) : string option =
  let rec aux (full_p : t_path) (p : t_path) (acc : string option) :
      string option =
    match (full_p, p) with
...
let rec string_of_shown_path (doc_settings : t_doc_settings)
    (full_path : t_path) (path : t_path) : string =
  ...
  | [], outer_path_hd :: _ -> (
      ...
      | CH_NODE _ -> (
          ...
          | Some (_, s) ->
              String.concat "\u{00A0}"
                [ s; string_of_path doc_settings full_outer_path ]
      ...
      | PAR_NODE par_node -> (
          ...
          | PAR_TAG (_, tag, _) ->
              String.concat "\u{00A0}"
                [ tag; string_of_path doc_settings full_outer_path ])
  ...
  | inner_path_hd :: _, _ :: _ -> (
      ...
      | ITM_NODE itm_node -> (
              ...
              | Some s ->
                  String.concat "\u{00A0}"
                    [
                      s;
                      "of";
                      string_of_shown_path doc_settings full_outer_path
                        full_outer_path;
                    ])
          | ITM_BIB_CUSTOM _ -> (
              match string_of_path_opt doc_settings full_path inner_path with
          ...
          | ITM_TAG_AUTO (tag, _) | ITM_TAG_CUSTOM (tag, _) -> (
              ...
              | Some s ->
                  String.concat "\u{00A0}"
                    [ tag; string_of_path doc_settings full_outer_path ^ s ]))
      | DSP_LINE_NODE dsp_line_node -> (
          match dsp_line_node with
          | DSP_AUTO _ | DSP_CUSTOM _ -> (
              match string_of_path_opt doc_settings full_path inner_path with
...
let string_of_tr_id (id : Doc_types.tr_id) : string =
  ...
  | Cs_tag tag, Cs_name name, Some scope ->
      String.concat ":" [ tag; name; string_of_tu_scope scope ]
  | Cs_tag tag, Cs_name name, None -> String.concat ":" [ tag; name ]

let rec string_of_ts_c_ref (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (c_ref_loc : t_path)
    (c_ref : Doc_types.ts_c_ref) : string =
  ...
  | Some (_, id_loc, _) -> (
      ...
      | hd :: tl -> (
          ...
          | ABSTRACT_NODE | REFS_NODE -> (
              match id_loc with
              | ITM_NODE (ITM_BIB_AUTO _) :: _
              | ITM_NODE (ITM_BIB_CUSTOM _) :: _ ->
              ...
              | _ ->
                  String.concat "\u{00A0}"
                    [
                      string_of_shown_path doc_settings id_loc (List.rev tl);
                      "of";
                      string_of_path doc_settings [ hd ];
                    ])
          | _ -> string_of_shown_path doc_settings id_loc sub_path)
      | [] -> raise (Error "path to id cannot be empty"))

let check_cref_table (doc_settings : t_doc_settings) (table : t_cref_table) :
    t_cref_table =
  let rec aux1 (lst : t_cref_table) (acc : (Doc_types.tr_id * t_path) list) :
      (Doc_types.tr_id * t_path) list =
    match lst with
    | [] -> acc
    | hd :: tl -> (
        match hd with
...
let label_of_path_opt (doc_settings : t_doc_settings) (path : t_path) :
    string option =
  match path with
  | [] -> None
  | hd :: tl -> (
      ...
      | PAR_NODE _ -> (
          ...
          | None, p -> Some p
          | Some (lbl, _), p -> Some (lbl ^ "\u{00A0}" ^ p))
      | ITM_NODE _ | BLT_NODE | DSP_LINE_NODE _ ->
          string_of_node_opt doc_settings tl hd
      | ABSTRACT_NODE -> (
          match doc_settings.abstract_hdr with
          | Some (lbl, _) -> Some lbl
          | None -> None)
...
let node_of_tr_par_std (doc_settings : t_doc_settings) (auto_nr : int)
    (par : Doc_types.tr_par_std) : t_node =
...
let node_of_tu_par (doc_settings : t_doc_settings) (auto_nr : int) (p : tu_par)
    : t_node =
  match p with
...
let node_of_blk_itm (doc_settings : t_doc_settings) (path : t_path)
    (auto_nr : int) (a : Doc_types.tr_blk_itm) : t_node =
...
let node_of_dsp_line (doc_settings : t_doc_settings) (path : t_path)
    (auto_nr : int) (a : Doc_types.tr_dsp_line) : t_node =
...
let txt_units_of_txt_lines (txt_lines : ts_txt_lines) : tu_txt_unit list =
  ...
  | Cs_txt_lines txt_line_list ->
      let rec aux (lines : ts_txt_line list) (acc : tu_txt_unit list) =
        ...
        | hd :: tl, _ ->
            aux tl (List.concat [ acc; [ space ]; txt_units_of_txt_line hd ])
      in
      aux txt_line_list []
...
let par_restated_of_tr_par (par : Doc_types.tr_par_std) : Doc_types.tr_par_std =
  ...
  let restated : tu_txt_unit =
    Cu_txt_unit_wysiwyg (Cs_txt_unit_wysiwyg "[restated]")
  in
                ...
                Some
                  (Cs_hdr
                     (Cs_txt_lines
                        [
                          Cs_txt_line
                            (Cs_txt_units [ txt_unit; space; restated ]);
                        ]))
                          ...
                          Cs_txt_line
                            (Cs_txt_units
                               (List.concat
                                  [
                                    [ txt_unit; space; lpar ];
                                    txt_units_of_txt_lines txt_lines;
                                    [ rpar; space; restated ];
                                  ]));
                        ]))))
  in
...
let par_restated_of_tr_id (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (id : tr_id) :
    (Doc_types.tr_par_std * t_path) option =
...
let utc_timezone ((sign, hour, minute) : string * int * int) : string =
  ...
  | _, _ ->
      "UTC" ^ sign ^ Printf.sprintf "%.2i" hour ^ ":"
      ^ Printf.sprintf "%.2i" minute

let time_of_ts_date_auto (doc_settings : t_doc_settings) (date : ts_date_auto) :
    t_time option =
  match date with
  | Cs_date_auto -> (
      try
        ...
        let local_minutes_ref : int =
          (local_time_ref.tm_hour * 60) + local_time_ref.tm_min
        in
        let gm_time_ref : Unix.tm = Unix.gmtime time_ref in
        let gm_minutes_ref : int =
          (gm_time_ref.tm_hour * 60) + gm_time_ref.tm_min
        in
        let diff_minutes : int =
          local_minutes_ref
          + (60 * Bool.to_int local_time.tm_isdst)
          - gm_minutes_ref
        in
      ...
      with _ ->
        let _ : unit =
          IO.print_warning "WARNING: cannot get system time and date."
        in
        None)
...
type t_nte_entry =
  | Ftn_entry_ref of (ts_nte_ref * t_path * int * tr_blk_nte)
  | Ftn_entry_inline of (ts_nte_inline * t_path * int)

type t_nte_table = t_nte_entry list

let reference_of_ts_nte_ref (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_path : t_path)
    (nte_ref : Doc_types.ts_nte_ref) : tr_blk_nte option =
  match nte_ref with
  | Cs_nte_ref (nte_id, i) ->
...
let nte_table_of_ts_txt_unit_nte_ref (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (txt_unit_nte_ref : ts_txt_unit_nte_ref) : t_nte_table =
  ...
  | Cs_txt_unit_nte_ref nte_ref -> (
      ...
      | Some blk_nte -> (
          ...
          | Ftn_entry_ref (_, _, n, _) :: _ ->
              Ftn_entry_ref (nte_ref, path, n + 1, blk_nte) :: nte_table
          | Ftn_entry_inline (_, _, n) :: _ ->
              Ftn_entry_ref (nte_ref, path, n + 1, blk_nte) :: nte_table))

let nte_table_of_ts_txt_unit_nte_inline (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (txt_unit_nte_inline : ts_txt_unit_nte_inline) : t_nte_table =
  match txt_unit_nte_inline with
  | Cs_txt_unit_nte_inline nte_inline -> (
      match nte_table with
      | [] -> Ftn_entry_inline (nte_inline, path, 0) :: nte_table
      | Ftn_entry_inline (_, _, n) :: _ ->
          Ftn_entry_inline (nte_inline, path, n + 1) :: nte_table
      | Ftn_entry_ref (_, _, n, _) :: _ ->
          Ftn_entry_inline (nte_inline, path, n + 1) :: nte_table)

let rec nte_table_of_tu_txt_unit_list (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (txt_unit_list : tu_txt_unit list) : t_nte_table =
  ...
  | hd :: tl -> (
      ...
      | Cu_txt_unit_nte_ref nte_ref ->
          let new_nte_table =
            nte_table_of_ts_txt_unit_nte_ref doc_settings cref_table path
              nte_table nte_ref
          in
          nte_table_of_tu_txt_unit_list doc_settings cref_table path
            new_nte_table tl
      | Cu_txt_unit_nte_inline nte_inline ->
          let new_nte_table =
            nte_table_of_ts_txt_unit_nte_inline doc_settings cref_table path
              nte_table nte_inline
          in
          nte_table_of_tu_txt_unit_list doc_settings cref_table path
            new_nte_table tl
      | _ ->
          nte_table_of_tu_txt_unit_list doc_settings cref_table path nte_table
            tl)

let nte_table_of_ts_txt_units (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (txt_units : ts_txt_units) : t_nte_table =
  match txt_units with
  | Cs_txt_units txt_unit_list ->
      nte_table_of_tu_txt_unit_list doc_settings cref_table path nte_table
        txt_unit_list

let nte_table_of_ts_txt_lines (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (txt_lines : ts_txt_lines) : t_nte_table =
  nte_table_of_ts_txt_units doc_settings cref_table path nte_table
    (Cs_txt_units (txt_units_of_txt_lines txt_lines))

let nte_table_of_ts_blk_txt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (blk_txt : ts_blk_txt) : t_nte_table =
  match blk_txt with
  | Cs_blk_txt txt_lines ->
      nte_table_of_ts_txt_lines doc_settings cref_table path nte_table txt_lines

let nte_table_of_tr_dsp_line (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (dsp_line : tr_dsp_line) : t_nte_table =
  nte_table_of_ts_txt_units doc_settings cref_table path nte_table
    dsp_line.fld_dsp_line_units

let string_of_ts_nte_ref (doc_settings : t_doc_settings)
    (nte_table : t_nte_table) (path : t_path) (nte_ref : ts_nte_ref) : string =
  match nte_ref with
  | Cs_nte_ref (id, i) ->
      let rec aux (table : t_nte_table) : string =
        ...
        | Ftn_entry_ref (table_nte_ref, _, n, _) :: tl ->
            if nte_ref = table_nte_ref then
              string_of_path doc_settings [ NTE_NODE n ]
            else aux tl
        | hd :: tl -> aux tl
      in
      aux nte_table

let string_of_ts_nte_inline (doc_settings : t_doc_settings)
    (nte_table : t_nte_table) (path : t_path) (nte_inline : ts_nte_inline) :
    string =
  ...
  | Cs_nte_inline (_, i) ->
      let rec aux (table : t_nte_table) : string =
        ...
        | Ftn_entry_inline (Cs_nte_inline (_, j), _, n) :: tl ->
            if i = j then string_of_path doc_settings [ NTE_NODE n ] else aux tl
        | hd :: tl -> aux tl
      in
      aux nte_table

let nte_table_of_ts_hdr (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (hdr : ts_hdr) =
  match hdr with
  | Cs_hdr txt_lines ->
...
let nte_table_of_ts_hdr_opt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_table : t_nte_table)
    (hdr_opt : ts_hdr option) =
  match hdr_opt with
  | None -> []
...
let exml_options_of_html_options (html_options : t_html_options) :
    t_exml_options =
  {
...
let axml_options_of_html_options (html_options : t_html_options) :
    t_axml_options =
  { tags = html_options.tags }
...
let axml_options_of_exml_options (exml_options : t_exml_options) :
    t_axml_options =
...
let axml_options_default () : t_axml_options = { tags = None }
