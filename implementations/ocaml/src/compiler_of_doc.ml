...
let rec acc_of_ts_blks (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (blks : ts_blks) : t_acc =
  let new_doc_settings =
    Common_utils.doc_settings_of_ts_blks doc_settings (lvl_of_path path) blks
  in
  let rec aux (auto_nr : int) (a : t_acc) (blk_list : tu_blk list) : t_acc =
    match blk_list with
    | [] -> a
    | hd :: tl -> (
        match
          acc_of_tu_blk new_doc_settings cref_table nte_table auto_nr path a hd
        with
        | (b : t_acc), (auto_nr : int) ->
  ...
  | Cs_blks (blk_list : tu_blk list) -> (
      ...
      | LINES _ ->
          let filter (blk : tu_blk) : tu_blk option =
            match blk with Cu_blk_nte _ -> None | _ -> Some blk
          in
          let filtered_blk_list : tu_blk list =
            List.filter_map filter blk_list
          in
          aux 0 acc filtered_blk_list
      | CREF_TABLE _ | EXML _ | MARGIN_LABELS _ | NTE_TABLE _ ->
          aux 0 acc blk_list)

and acc_of_tu_blk (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (auto_nr : int) (path : t_path) (acc : t_acc)
    (blk : tu_blk) : t_acc * int =
  match blk with
  | Cu_blk_itm (blk_itm : tr_blk_itm) ->
      let node : t_node =
        Common_utils.node_of_blk_itm doc_settings path auto_nr blk_itm
      in
      let next_auto_nr =
        ...
        | _ -> auto_nr
      in
      ( acc_of_tr_blk_itm doc_settings cref_table nte_table (node :: path) acc
          blk_itm,
        next_auto_nr )
  | Cu_blk_dsp (blk_dsp : ts_blk_dsp) ->
      acc_of_ts_blk_dsp doc_settings cref_table nte_table auto_nr
        (DSP_NODE :: path) acc blk_dsp
  | Cu_blk_txt (blk_txt : ts_blk_txt) ->
      ( acc_of_ts_blk_txt doc_settings cref_table nte_table path acc blk_txt,
        auto_nr )
  | Cu_blk_blt (blk_blt : ts_blk_blt) ->
      ( acc_of_ts_blk_blt doc_settings cref_table nte_table (BLT_NODE :: path)
          acc blk_blt,
        auto_nr )
  | Cu_blk_vrb (blk_vrb : ts_blk_vrb) ->
      (acc_of_ts_blk_vrb doc_settings path acc blk_vrb, auto_nr)
  | Cu_blk_nte (blk_nte : tr_blk_nte) ->
      (acc_of_tr_blk_nte doc_settings cref_table path acc blk_nte, auto_nr)
  | Cu_blk_qtn (blk_qtn : ts_blk_qtn) ->
      (acc_of_ts_blk_qtn doc_settings path acc blk_qtn, auto_nr)

(* blk_qtn *)

and acc_of_ts_blk_qtn (doc_settings : t_doc_settings) (path : t_path)
    (acc : t_acc) (blk_qtn : ts_blk_qtn) : t_acc =
  match acc with
  | MARGIN_LABELS _ | CREF_TABLE _ | NTE_TABLE _ -> acc
  | LINES acc_lines ->
      let lines : string list =
        Txt_utils.lines_of_ts_blk_qtn doc_settings path blk_qtn
      in
      LINES (List.concat [ acc_lines; lines ])
  | EXML acc_list ->
      let exml : Xml.xml = Exml_utils.xml_of_ts_blk_qtn blk_qtn in
      EXML (List.concat [ acc_list; [ exml ] ])

(* blk_txt *)

and acc_of_ts_blk_txt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (blk_txt : ts_blk_txt) : t_acc =
  match acc with
  | MARGIN_LABELS _ | CREF_TABLE _ -> acc
  | LINES acc_lines ->
      let lines : string list =
        Txt_utils.lines_of_ts_blk_txt doc_settings cref_table nte_table path
          blk_txt
      in
      LINES (List.concat [ acc_lines; lines ])
  | EXML acc_list ->
      let exml : Xml.xml =
        Exml_utils.xml_of_ts_blk_txt doc_settings cref_table nte_table path
          blk_txt
      in
      EXML (List.concat [ acc_list; [ exml ] ])
  | NTE_TABLE acc_table ->
      NTE_TABLE
        (Common_utils.nte_table_of_ts_blk_txt doc_settings cref_table path
           acc_table blk_txt)

(* blk_dsp *)

and acc_of_ts_blk_dsp (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (auto_nr : int)
    (path : t_path) (acc : t_acc) (blk_dsp : ts_blk_dsp) : t_acc * int =
  let rec aux (auto_nr : int) (a : t_acc) (dsp_line_list : tr_dsp_line list) :
      t_acc * int =
    match dsp_line_list with
    | [] -> (a, auto_nr)
    | hd :: tl ->
        let node : t_node =
          Common_utils.node_of_dsp_line doc_settings path auto_nr hd
        in
        let next_auto_nr =
        ...
        in
        let b : t_acc =
          acc_of_tr_dsp_line doc_settings cref_table nte_table (node :: path) a
            hd
        in
  ...
  | Cs_blk_dsp (dsp_lines : ts_dsp_lines) -> (
      ...
      | Cs_dsp_lines (dsp_line_list : tr_dsp_line list) -> (
          ...
          | CREF_TABLE _ | NTE_TABLE _ -> aux auto_nr acc dsp_line_list
          | LINES acc_lines -> (
              match aux auto_nr (LINES []) dsp_line_list with
              | LINES lines, nr -> (LINES (List.concat [ acc_lines; lines ]), nr)
              | _ -> raise (Error "accumulator type"))
          | EXML acc_list -> (
              match aux auto_nr (EXML []) dsp_line_list with
              | EXML xml_list, nr ->
                  let exml : Xml.xml = Xml.Element ("blk_dsp", [], xml_list) in
                  (EXML (List.concat [ acc_list; [ exml ] ]), nr)
              | _ -> raise (Error "accumulator type"))))

and acc_of_tr_dsp_line (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (dsp_line : tr_dsp_line) : t_acc =
  match acc with
  | MARGIN_LABELS _ -> acc
  | CREF_TABLE table -> (
      match dsp_line.fld_dsp_line_id with
      | Some (id : tr_id) ->
          CREF_TABLE ((id, path, Cref_element_dsp_line dsp_line) :: table)
      | None -> acc)
  | LINES acc_lines ->
      let lines : string list =
        Txt_utils.lines_of_tr_dsp_line doc_settings cref_table nte_table path
          dsp_line
      in
      LINES (List.concat [ acc_lines; lines ])
  | EXML acc_list ->
      let exml : Xml.xml =
        Exml_utils.xml_of_tr_dsp_line doc_settings cref_table nte_table path
          dsp_line
      in
      EXML (List.concat [ acc_list; [ exml ] ])
  | NTE_TABLE acc_table ->
      let table : t_nte_table =
        Common_utils.nte_table_of_tr_dsp_line doc_settings cref_table path
          acc_table dsp_line
      in
      NTE_TABLE table
...
and acc_of_ts_blk_vrb (doc_settings : t_doc_settings) (path : t_path)
    (acc : t_acc) (blk_vrb : ts_blk_vrb) : t_acc =
  match acc with
  | NTE_TABLE _ | MARGIN_LABELS _ | CREF_TABLE _ -> acc
  | LINES acc_lines ->
      let lines : string list =
      ...
      LINES (List.concat [ acc_lines; lines ])
  | EXML acc_list ->
      let exml : Xml.xml = Exml_utils.xml_of_ts_blk_vrb blk_vrb in
      EXML (List.concat [ acc_list; [ exml ] ])
...
and acc_of_tr_blk_nte (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (acc : t_acc)
    (blk_nte : tr_blk_nte) : t_acc =
  match acc with
  | CREF_TABLE table ->
      CREF_TABLE
        ((blk_nte.fld_blk_nte_id, path, Cref_element_blk_nte blk_nte) :: table)
  | _ -> acc

and lines_of_nte_blks (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (blks : ts_blks) : string list =
  let new_cref_table =
    match
      acc_of_ts_blks doc_settings [] [] path (CREF_TABLE cref_table) blks
    with
...
and lines_of_nte_table (doc_settings : t_doc_settings)
  ...
  let map (nte_entry : t_nte_entry) : string list option =
    ...
    | Ftn_entry_ref (_, table_path, n, blk_nte) -> (
        ...
        | true ->
            Some
              (lines_of_nte_blks doc_settings cref_table (NTE_NODE n :: path)
    ...
    | Ftn_entry_inline (Cs_nte_inline (blks, _), table_path, n) -> (
        ...
        | false -> None)
  in
  let rec aux1 (table : t_nte_table) (acc : string list list) : string list list
      =
    match table with
    | [] -> acc
    | hd :: tl -> (
  ...
  let rec aux2 (string_list_list : string list list) (acc : string list) :
      string list =
...
and xml_of_blk_nte_inline (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (nte_inline : ts_nte_inline) :
    Xml.xml =
  ...
  | Cs_nte_inline (blks, Cs_int i) ->
      ...
      let xml_list_main : Xml.xml list =
        match
          acc_of_ts_blks doc_settings new_cref_table [] path (EXML []) blks
        with
...
and xml_of_blk_nte_ref (doc_settings : t_doc_settings)
    ...
    (blk_nte : tr_blk_nte) : Xml.xml =
  let new_cref_table =
    match
      acc_of_ts_blks doc_settings [] [] path (CREF_TABLE cref_table)
        blk_nte.fld_blk_nte_main
    with
    | CREF_TABLE table -> table
    | _ -> raise (Error "accumulator type")
  ...
  let xml_list_main : Xml.xml list =
    match
      acc_of_ts_blks doc_settings new_cref_table [] path (EXML [])
        blk_nte.fld_blk_nte_main
    with
    | EXML xml_list -> xml_list
    | _ -> raise (Error "accumulator type")
  in
  let addendum : string =
    match nte_ref with Cs_nte_ref (id, Cs_int i) -> string_of_int i
  in
  ...
  in
  let attr_list_lbl : (string * string) list =
    match attr_list with [ ("id", s) ] -> [ ("href", "#ref_" ^ s) ] | _ -> []
  in
  let xml_lbl : Xml.xml =
    Xml.Element ("blk_nte_lbl", attr_list_lbl, xml_list_lbl)
  in
...
and xml_of_nte_table_opt (doc_settings : t_doc_settings)
  ...
  let map (nte_entry : t_nte_entry) : Xml.xml option =
    ...
    | Ftn_entry_ref (nte_ref, table_path, n, blk_nte) -> (
        ...
        | true ->
            Some
              (xml_of_blk_nte_ref doc_settings cref_table (NTE_NODE n :: path)
                 nte_ref blk_nte)
        | false -> None)
  ...
  let rec aux (table : t_nte_table) (acc : Xml.xml list) : Xml.xml list =
    ...
    | [] -> acc
    | hd :: tl -> (
        match map hd with None -> aux tl acc | Some xml -> aux tl (xml :: acc))
  in
  let xml_list : Xml.xml list = aux nte_table [] in
  let xml_hdr_opt : Xml.xml option =
    match (doc_settings.endnotes_hdr, path) with
    | Some hdr, [] ->
        Some (Xml.Element ("doc_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, CH_NODE _ :: _ ->
        Some (Xml.Element ("ch_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, SEC_NODE _ :: _ ->
        Some (Xml.Element ("sec_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, APP_NODE _ :: _ ->
        Some (Xml.Element ("app_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, PAR_NODE _ :: _ ->
        Some (Xml.Element ("par_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, ABSTRACT_NODE :: _ ->
        Some (Xml.Element ("abstract_endnotes_hdr", [], [ xml_of_string hdr ]))
    | Some hdr, REFS_NODE :: _ ->
        Some (Xml.Element ("refs_endnotes_hdr", [], [ xml_of_string hdr ]))
    | _, _ -> None
  in
  match (xml_hdr_opt, xml_list, path) with
  | Some hdr, _ :: _, [] ->
      Some (Xml.Element ("doc_endnotes", [], hdr :: xml_list))
  | None, _ :: _, [] -> Some (Xml.Element ("doc_endnotes", [], xml_list))
  | Some hdr, _ :: _, CH_NODE _ :: _ ->
      Some (Xml.Element ("ch_endnotes", [], hdr :: xml_list))
  | None, _ :: _, CH_NODE _ :: _ ->
      Some (Xml.Element ("ch_endnotes", [], xml_list))
  | Some hdr, _ :: _, SEC_NODE _ :: _ ->
      Some (Xml.Element ("sec_endnotes", [], hdr :: xml_list))
  | None, _ :: _, SEC_NODE _ :: _ ->
      Some (Xml.Element ("sec_endnotes", [], xml_list))
  | None, _ :: _, APP_NODE _ :: _ ->
      Some (Xml.Element ("app_endnotes", [], xml_list))
  | Some hdr, _ :: _, APP_NODE _ :: _ ->
      Some (Xml.Element ("app_endnotes", [], hdr :: xml_list))
  | None, _ :: _, PAR_NODE _ :: _ ->
      Some (Xml.Element ("par_endnotes", [], xml_list))
  | Some hdr, _ :: _, PAR_NODE _ :: _ ->
      Some (Xml.Element ("par_endnotes", [], hdr :: xml_list))
  | None, _ :: _, ABSTRACT_NODE :: _ ->
      Some (Xml.Element ("abstract_endnotes", [], xml_list))
  | Some hdr, _ :: _, ABSTRACT_NODE :: _ ->
      Some (Xml.Element ("abstract_endnotes", [], hdr :: xml_list))
  | None, _ :: _, REFS_NODE :: _ ->
      Some (Xml.Element ("refs_endnotes", [], xml_list))
  | Some hdr, _ :: _, REFS_NODE :: _ ->
      Some (Xml.Element ("refs_endnotes", [], hdr :: xml_list))
  | _, _, _ -> None

(* blk_itm *)

and acc_of_tr_blk_itm (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (blk_itm : tr_blk_itm) : t_acc =
  match acc with
  | NTE_TABLE _ ->
      acc_of_ts_blks doc_settings cref_table nte_table path acc
  ...
  | CREF_TABLE table ->
      let new_acc : t_acc =
        CREF_TABLE
          ...
          | None -> table)
      in
      acc_of_ts_blks doc_settings cref_table nte_table path new_acc
        blk_itm.fld_blk_itm_main
  | LINES acc_lines -> (
      match
        acc_of_ts_blks doc_settings cref_table nte_table path (LINES [])
          blk_itm.fld_blk_itm_main
      with
  ...
  | EXML acc_list ->
      let xml_list_main =
        match
          acc_of_ts_blks doc_settings cref_table nte_table path (EXML [])
            blk_itm.fld_blk_itm_main
        with
      ...
      in
      let attr_list : (string * string) list =
        Exml_utils.attr_list_of_tu_tag_or_id_opt doc_settings path
          ("blk" :: "itm" :: classes)
...
and acc_of_ts_blk_blt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (blk_blt : ts_blk_blt) : t_acc =
  match blk_blt with
  | Cs_blk_blt (blks : ts_blks) -> (
      match acc with
      | MARGIN_LABELS _ -> acc
      | NTE_TABLE _ | CREF_TABLE _ ->
      ...
      | LINES acc_lines -> (
          ...
          | LINES (lines_hd :: lines_tl : string list) ->
              ...
              in
              let new_lines : string list = new_head :: lines_tl in
...
let acc_of_par_main (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (blks : ts_blks) :
    t_acc =
  acc_of_ts_blks doc_settings cref_table nte_table path acc blks

let acc_of_tr_par_std (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (path_origin : t_path) (acc : t_acc) (par_std : tr_par_std) : t_acc =
  match acc with
  | NTE_TABLE acc_table -> (
      let table_hdr : t_nte_table =
        Common_utils.nte_table_of_ts_hdr_opt doc_settings cref_table path []
          par_std.fld_par_hdr
      in
      match
  ...
  | CREF_TABLE table ->
      let newacc : t_acc =
        CREF_TABLE
          ...
          | _ -> table)
      in
      acc_of_ts_blks doc_settings cref_table nte_table path newacc
        par_std.fld_par_main
  | LINES acc_lines -> (
      let new_par = Txt_utils.copy_hdr_to_main doc_settings par_std in
      let lines_endnotes : string list =
        lines_of_nte_table doc_settings cref_table path nte_table
      in
      match
        acc_of_ts_blks doc_settings cref_table nte_table path_origin (LINES [])
  ...
  | EXML acc_list ->
      let xml_list_hdr_opt : Xml.xml list option =
        Exml_utils.par_hdr_opt doc_settings cref_table nte_table path_origin
          par_std.fld_par_tag_or_id par_std.fld_par_hdr
      in
      let xml_list_lbl : Xml.xml list =
        [ Exml_utils.xml_of_string (label_of_path doc_settings path) ]
      in
      ...
      in
      let attr_list : (string * string) list =
        Exml_utils.attr_list_of_tu_tag_or_id_opt doc_settings path [ "par" ]
...
let acc_of_tu_par (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (par : tu_par) :
    t_acc =
  match par with
  | Cu_par_std (par_std : tr_par_std) ->
      acc_of_tr_par_std doc_settings cref_table nte_table path path acc par_std
  | Cu_par_rpt (Cs_par_rpt (id : tr_id)) -> (
      match acc with
      | MARGIN_LABELS string_list ->
          MARGIN_LABELS (label_of_path doc_settings path :: string_list)
      | NTE_TABLE _ | CREF_TABLE _ -> acc
      | _ -> (
          match par_restated_of_tr_id doc_settings cref_table path id with
          | Some ((par_std : tr_par_std), (path_origin : t_path)) ->
              acc_of_tr_par_std doc_settings cref_table nte_table path
                path_origin acc par_std
          | None ->
              let _ : unit =
                IO.print_warning
                  (String.concat ""
...
let acc_of_ts_pars (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (pars : ts_pars) :
    t_acc =
  let rec aux (par_nr : int) (a : t_acc) (par_list : tu_par list) : t_acc =
    match par_list with
    | [] -> a
    | hd :: tl ->
        let new_path : t_path =
        ...
        in
        let new_acc : t_acc =
          add_empty_lines_after_par tl
            (acc_of_tu_par doc_settings cref_table nte_table new_path a hd)
        in
        aux (par_nr + 1) new_acc tl
  in
  match pars with Cs_pars (par_list : tu_par list) -> aux 0 acc par_list

(* secs *)

let acc_of_sec_main (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc)
    (pars_or_blks : tu_pars_or_blks) : t_acc =
...
let acc_of_tr_sec (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (sec : tr_sec) :
    t_acc =
  match acc with
  | NTE_TABLE acc_table -> (
      let table_hdr : t_nte_table =
      ...
      in
      match
        acc_of_sec_main doc_settings cref_table nte_table path
          (NTE_TABLE table_hdr) sec.fld_sec_main
      with
      | NTE_TABLE table -> NTE_TABLE (List.concat [ table; acc_table ])
      | _ -> raise (Error "accumulator type"))
  | MARGIN_LABELS string_list ->
  ...
  | CREF_TABLE table ->
      let newacc : t_acc =
        CREF_TABLE
          ...
          | _ -> table)
      in
      acc_of_sec_main doc_settings cref_table nte_table path newacc
        sec.fld_sec_main
  | LINES acc_lines ->
      let lines : string list =
        ...
        | None, Cu_pars_or_blks_blks _ -> (
            match
              acc_of_sec_main doc_settings cref_table nte_table path (LINES [])
                sec.fld_sec_main
            with
        ...
        | _, _ -> (
            let lines_hdr : string list =
              Txt_utils.lines_of_ts_hdr_opt doc_settings cref_table nte_table
                path sec.fld_sec_hdr
            in
            match
              acc_of_sec_main doc_settings cref_table nte_table path (LINES [])
                sec.fld_sec_main
  ...
  | EXML acc_list ->
      let xml_list_main : Xml.xml list =
        match
          acc_of_sec_main doc_settings cref_table nte_table path (EXML [])
            sec.fld_sec_main
        with
        | EXML xml_list -> xml_list
        | _ -> raise (Error "accumulator type")
      ...
      let attr_list : (string * string) list =
        Exml_utils.attr_list_of_tu_tag_or_id_opt doc_settings path
          [ "sec"; sec_class ] sec.fld_sec_tag_or_id
      in
      let exml : Xml.xml =
        match (sec.fld_sec_hdr, xml_endnotes_opt) with
        | None, None -> Xml.Element ("sec", attr_list, [ xml_hdr; xml_main ])
        | Some _, None ->
        ...
        | Some _, Some endnotes ->
            Xml.Element
              ("sec", attr_list, [ xml_lbl; xml_hdr; xml_main; endnotes ])
      in
      EXML (List.concat [ acc_list; [ exml ] ])
...
let is_appendix (sec : tr_sec) : bool =
  ...
  | Some (tag_or_id : tu_tag_or_id) -> (
      ...
      | Cu_tag_or_id_tag (tag : ts_tag) -> (
          match tag with
          | Cs_tag (s : string) -> ( match s with "APP" -> true | _ -> false))
      | Cu_tag_or_id_id (id : tr_id) -> (
          match id.fld_id_tag with
          | Cs_tag (s : string) -> ( match s with "APP" -> true | _ -> false)))

let acc_of_ts_secs (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (secs : ts_secs) :
    t_acc =
  let rec aux (sec_nr : int) (app_nr : int) (a : t_acc) (sec_list : tr_sec list)
      : t_acc =
    match sec_list with
    | [] -> a
    | hd :: tl -> (
  ...
  in
  match secs with Cs_secs (sec_list : tr_sec list) -> aux 0 0 acc sec_list

(* chs *)

let acc_of_ch_main (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc)
    (ch_main : tu_secs_pars_or_blks) : t_acc =
  match ch_main with
...
let acc_of_tr_ch (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (ch : tr_ch) : t_acc
    =
  match acc with
  | NTE_TABLE acc_table -> (
      let table_hdr : t_nte_table =
      ...
      in
      match
        acc_of_ch_main doc_settings cref_table nte_table path
          (NTE_TABLE table_hdr) ch.fld_ch_main
      with
      | NTE_TABLE table -> NTE_TABLE (List.concat [ table; acc_table ])
      | _ -> raise (Error "accumulator type"))
  | MARGIN_LABELS _ ->
      acc_of_ch_main doc_settings cref_table nte_table path acc ch.fld_ch_main
  | CREF_TABLE table ->
      let newacc : t_acc =
        CREF_TABLE
          ...
          | _ -> table)
      in
      acc_of_ch_main doc_settings cref_table nte_table path newacc
        ch.fld_ch_main
  | LINES acc_lines ->
      let lines_hdr : string list =
        Txt_utils.lines_of_ts_hdr_opt doc_settings cref_table nte_table path
          ch.fld_ch_hdr
      in
      let lines_main : string list =
        match
          acc_of_ch_main doc_settings cref_table nte_table path (LINES [])
            ch.fld_ch_main
        with
        | LINES lines -> lines
        | _ -> raise (Error "accumulator type")
  ...
  | EXML acc_list ->
      let xml_list_main : Xml.xml list =
        match
          acc_of_ch_main doc_settings cref_table nte_table path (EXML [])
            ch.fld_ch_main
        with
        | EXML xml_list -> xml_list
        | _ -> raise (Error "accumulator type")
        ...
        | Some (hdr : ts_hdr) -> (
            ...
            | Cs_hdr (txt_lines : ts_txt_lines) ->
                Xml.Element
                  ( "ch_hdr",
                    [],
                    Exml_utils.xml_list_of_ts_txt_lines doc_settings cref_table
                      nte_table path txt_lines ))
      in
      let xml_endnotes_opt : Xml.xml option =
        match
          ( ch.fld_ch_main,
      ...
      let attr_list : (string * string) list =
        Exml_utils.attr_list_of_tu_tag_or_id_opt doc_settings path [ "ch" ]
          ch.fld_ch_tag_or_id
      in
      let xml_list_ch =
      ...
      in
      let exml : Xml.xml = Xml.Element ("ch", attr_list, xml_list_ch) in
      EXML (List.concat [ acc_list; [ exml ] ])
...
let acc_of_ts_chs (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (chs : ts_chs) :
    t_acc =
  let rec aux (ch_nr : int) (a : t_acc) (ch_list : tr_ch list) : t_acc =
    match ch_list with
    | [] -> a
    | hd :: tl ->
  ...
  in
  match chs with Cs_chs (ch_list : tr_ch list) -> aux 0 acc ch_list

(* doc *)

let acc_of_ts_abstract (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table)
    (doc_class : t_doc_class) (path : t_path) (acc : t_acc)
    (abstract : ts_abstract) : t_acc =
  match abstract with
  | Cs_abstract (blks : ts_blks) -> (
      ...
      | LINES _ -> (
          ...
          in
          let hdr : string list =
            Txt_utils.lines_of_abstract_hdr doc_settings doc_class
          in
      ...
      | EXML _ ->
          ...
          in
          EXML [ exml ]
      | CREF_TABLE _ | MARGIN_LABELS _ | NTE_TABLE _ ->
          acc_of_ts_blks doc_settings cref_table nte_table path acc blks)

let acc_of_ts_refs (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (doc_class : t_doc_class) (path : t_path)
    (acc : t_acc) (refs : ts_refs) : t_acc =
  match refs with
  | Cs_refs (blks : ts_blks) -> (
      ...
      | EXML _ -> (
          ...
          in
          match
            acc_of_ts_blks doc_settings cref_table nte_table path (EXML []) blks
          with
          | EXML xml_list ->
              EXML
                [
                  Xml.Element
                    ("refs", [], List.concat [ hdr; xml_list; endnotes ]);
                ]
          | _ -> raise (Error "accumulator type"))
      | CREF_TABLE _ | MARGIN_LABELS _ | NTE_TABLE _ ->
          acc_of_ts_blks doc_settings cref_table nte_table path acc blks)

let acc_of_tu_doc_main (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (acc : t_acc) (doc_main : tu_doc_main) : t_acc =
  match doc_main with
...
let acc_of_tr_doc (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (nte_table : t_nte_table) (path : t_path) (acc : t_acc) (doc : tr_doc) :
    t_acc =
  let doc_class : t_doc_class = class_of_tr_doc doc in
  match acc with
  | MARGIN_LABELS _ ->
      acc_of_tu_doc_main doc_settings cref_table nte_table path acc
        doc.fld_doc_main
  | NTE_TABLE _ ->
      let table_abstract : t_nte_table =
        ...
        | Some (abstract : ts_abstract) -> (
            match
              acc_of_ts_abstract doc_settings cref_table nte_table doc_class
                (ABSTRACT_NODE :: path) acc abstract
            with
      ...
      let table_refs : t_nte_table =
        ...
        | Some (refs : ts_refs) -> (
            match
              acc_of_ts_refs doc_settings cref_table nte_table doc_class
                (REFS_NODE :: path) acc refs
            with
            | NTE_TABLE table -> table
            | _ -> raise (Error "accumulator type"))
      in
        ...
        | DOC_BLKS -> (
            match
              acc_of_tu_doc_main doc_settings cref_table nte_table path
                (NTE_TABLE table_abstract) doc.fld_doc_main
            with
            | NTE_TABLE table -> table
            | _ -> raise (Error "accumulator type"))
        | _ -> (
  ...
  | EXML _ ->
      ...
      EXML
        [ Xml.Element ("doc", [ ("class", doc_class_string) ], xml_list_doc) ]

(* margin labels *)

let margin_labels_of_tr_doc (doc_settings : t_doc_settings) (doc : tr_doc) :
    string list =
  let _ : unit = IO.quiet.contents <- true in
  match
    acc_of_tr_doc doc_settings
      ([] : t_cref_table)
      ([] : t_nte_table)
      ([] : t_path)
      (MARGIN_LABELS []) doc
...
let cref_table_of_tr_doc (doc_settings : t_doc_settings) (doc : tr_doc) :
    t_cref_table =
  let _ : unit = IO.quiet.contents <- true in
  match
    acc_of_tr_doc doc_settings
...
let nte_table_of_tr_doc (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (doc : tr_doc) : t_nte_table =
  let _ : unit = IO.quiet.contents <- true in
  match
    acc_of_tr_doc doc_settings cref_table
      ([] : t_nte_table)
      ([] : t_path)
      (NTE_TABLE []) doc
  with
...
let lines_of_tr_doc (doc_settings : t_doc_settings) (doc : tr_doc) : string list
  ...
  match
    acc_of_tr_doc doc_settings cref_table nte_table ([] : t_path) (LINES []) doc
  with
  | LINES lines -> lines
  | _ -> raise (Error "accumulator type")
...
let xml_list_of_tr_doc (doc_settings : t_doc_settings) (doc : tr_doc) :
    Xml.xml list =
  let quiet : bool = IO.quiet.contents in
  let cref_table : t_cref_table = cref_table_of_tr_doc doc_settings doc in
  let nte_table : t_nte_table =
  ...
  let _ : unit = IO.quiet.contents <- quiet in
  match
    acc_of_tr_doc doc_settings cref_table nte_table ([] : t_path) (EXML []) doc
  with
  | EXML xml_list -> xml_list
  | _ -> raise (Error "accumulator type")
