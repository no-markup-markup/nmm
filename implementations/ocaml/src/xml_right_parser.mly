...
let first (pair : 'a * 'a) : 'a = match pair with x, _ -> x
let second (pair : 'a * 'a) : 'a = match pair with _, y -> y
...
let attr_list_of_string (s : string) : (string * string) list =
  ...
  | _ ->
      ...
      let rec pair (x : string list) : (string * string) list =
        ...
        | [] | [ "" ] -> []
        | a :: b :: tl ->
            (String.concat "" (String.split_on_char '=' a), b) :: pair tl
        | _ -> raise (Error (String.concat "%" trimmed_str_list))
      in
      pair trimmed_str_list
%}
