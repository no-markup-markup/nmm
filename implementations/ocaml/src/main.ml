...
let version () : string = "2.5"

(* parsing nmm *)

let doc_of_nmm (options : Common_utils.t_axml_options) (path : string) :
    Doc_types.tr_doc =
  try
    match path with
    | "-" ->
        Doc_of_nmm.doc_of_nmm_stdin_with_tagger
          (Tags.tagger_of_path_opt options.tags)
    | _ ->
        Doc_of_nmm.doc_of_nmm_file_with_tagger
          (Tags.tagger_of_path_opt options.tags)
          path
  with Doc_of_nmm.Error e ->
    raise (Error (String.concat " " [ path; "->"; "Doc_of_nmm.Error:"; e ]))

(* txt semantics *)

let txt_of_doc (options : Common_utils.t_txt_options) (doc : Doc_types.tr_doc) :
    string =
  try Compiler_of_doc.txt_of_tr_doc options doc
  with Compiler_of_doc.Error e ->
    raise (Error (String.concat " " [ "Compiler_of_doc.Error:"; e ]))

let txt_of_nmm (options : Common_utils.t_txt_options) (path : string) : string =
  txt_of_doc options
    (doc_of_nmm (Common_utils.axml_options_of_txt_options options) path)
...
let html_of_doc (options : Common_utils.t_html_options) (doc : Doc_types.tr_doc)
  ...
  | Html_utils.Error e ->
      raise (Error (String.concat " " [ "Html_utils.Error:"; e ]))
  | Compiler_of_doc.Error e ->
      raise (Error (String.concat " " [ "Compiler_of_doc.Error:"; e ]))
  | Xml_right.Error e ->
      raise (Error (String.concat " " [ "Xml_right.Error:"; e ]))

let html_of_nmm (options : Common_utils.t_html_options) (path : string) : string
    =
  html_of_doc options
    (doc_of_nmm (Common_utils.axml_options_of_html_options options) path)
...
let doc_of_axml (path : string) : Doc_types.tr_doc =
  ...
  | Doc_of_axml.Error e ->
      raise (Error (String.concat " " [ path; "->"; "Doc_of_axml.Error:"; e ]))

let axml_of_doc (doc : Doc_types.tr_doc) : string =
  "<?xml version=\"1.0\"?>\n"
  ^ Xml_right.to_string_fmt
      (Axml_of_doc.normalize_axml (Axml_of_doc.axml_of_tr_doc doc))
...
let exml_of_doc (options : Common_utils.t_exml_options) (doc : Doc_types.tr_doc)
    : string =
  "<?xml version=\"1.0\"?>\n"
  ^ Xml_right.to_string_fmt
      (Exml_utils.normalize_exml (Compiler_of_doc.exml_of_tr_doc options doc))

let exml_of_nmm (options : Common_utils.t_exml_options) (path : string) : string
    =
  exml_of_doc options
    (doc_of_nmm (Common_utils.axml_options_of_exml_options options) path)

let exml_of_axml (options : Common_utils.t_exml_options) (path : string) :
    string =
...
let validate_xml (path_to_dtd : string) (path_to_xml : string) : string =
  ...
  | Xml_light_errors.Dtd_prove_error e ->
      raise
        (Error
           (String.concat " "
              [
  ...
  | Xml_light_errors.Xml_error e ->
      raise
        (Error
           (String.concat " "
              [ path_to_xml; "->"; "Xml_light_errors.Xml_error:"; Xml.error e ]))
  | Xml_light_errors.File_not_found e ->
      raise
        (Error (String.concat " " [ "Xml_light_errors.File_not_found:"; e ]))
  | Xml_right.Error e ->
      raise
