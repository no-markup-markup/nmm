...
let xml_of_ts_date_auto (doc_settings : t_doc_settings) (date : ts_date_auto) :
    Xml.xml option =
  ...
  | Some (time : Common_utils.t_time) ->
      ...
      let datetime_string : string =
        String.concat ""
          [ date_string; "T"; time_string; string_of_timezone time.timezone ]
      in
      Some
        (Xml.Element
           ( "date",
             [ ("datetime", datetime_string) ],
             [ xml_of_string display_string ] ))

let xml_of_ts_date_custom (date : ts_date_custom) : Xml.xml =
  match date with
  | Cs_date_custom s ->
      Xml.Element ("date", [ ("datetime", s) ], [ xml_of_string s ])

let xml_of_tu_date (doc_settings : t_doc_settings) (date : tu_date) :
    Xml.xml option =
  match date with
  | Cu_date_auto d -> xml_of_ts_date_auto doc_settings d
  | Cu_date_custom d -> Some (xml_of_ts_date_custom d)

let xml_list_of_tu_date_opt (doc_settings : t_doc_settings)
    (date_opt : tu_date option) : Xml.xml list =
  match date_opt with
  | None -> []
  | Some date -> (
...
let xml_list_of_abstract_hdr (doc_settings : t_doc_settings) : Xml.xml list =
  ...
  | None -> []
  | Some (abstract_hdr, _) ->
      [ Xml.Element ("abstract_hdr", [], [ xml_of_string abstract_hdr ]) ]
...
let cdata_of_string (s : string) : string = Xml_right.pcdata_of_string s

let string_of_scope (doc_settings : t_doc_settings) (path : t_path)
    (scope : tu_scope) : string =
  match scope with
  | Cu_scope_gbl -> "GBL"
...
let cdata_of_tr_id (doc_settings : t_doc_settings) (path : t_path) (id : tr_id)
    : string =
  match (id.fld_id_tag, id.fld_id_name, id.fld_id_scope) with
  | Cs_tag (tag_string : string), Cs_name (name_string : string), None ->
      cdata_of_string (tag_string ^ "_" ^ name_string)
  | Cs_tag (tag_string : string), Cs_name (name_string : string), Some scope ->
      cdata_of_string
        (tag_string ^ "_" ^ name_string ^ "_"
        ^ string_of_scope doc_settings path scope)

let attr_list_of_tr_id (doc_settings : t_doc_settings) (path : t_path)
    (id : tr_id) : (string * string) list =
  [ ("id", cdata_of_tr_id doc_settings path id) ]

let attr_list_of_ts_tag (classes : string list) (tag : ts_tag) :
    (string * string) list =
  match tag with
  | Cs_tag (s : string) -> [ ("class", String.concat " " (s :: classes)) ]

let attr_list_of_tr_id_opt (doc_settings : t_doc_settings) (path : t_path)
    (classes : string list) (id_opt : tr_id option) : (string * string) list =
  match id_opt with
  | None -> [ ("class", String.concat " " classes) ]
  | Some id ->
      List.concat
        [
          attr_list_of_ts_tag classes id.fld_id_tag;
          attr_list_of_tr_id doc_settings path id;
        ]

let attr_list_of_tu_tag_or_id_opt (doc_settings : t_doc_settings)
    (path : t_path) (classes : string list) (a : tu_tag_or_id option) :
    (string * string) list =
  match a with
  | None -> (
      match classes with
...
let attr_list_of_ts_c_ref (doc_settings : t_doc_settings) (path : t_path)
    (a : ts_c_ref) : (string * string) list =
  match a with
  | Cs_c_ref (id : tr_id) ->
      [ ("href", "#" ^ cdata_of_tr_id doc_settings path id) ]

let xml_of_ts_c_ref (doc_settings : t_doc_settings) (cref_table : t_cref_table)
    (path : t_path) (a : ts_c_ref) : Xml.xml =
  xml_of_string (string_of_ts_c_ref doc_settings cref_table path a)

(* nte_ref *)

let attr_list_of_ts_nte_ref (doc_settings : t_doc_settings) (path : t_path)
    (a : ts_nte_ref) : (string * string) list =
  match a with
  | Cs_nte_ref (id, Cs_int i) ->
      let addendum : string = string_of_int i in
      let href_string : string =
        cdata_of_tr_id doc_settings path id ^ "_" ^ addendum
      in
      let id_string : string = "ref_" ^ href_string in
      [ ("href", "#" ^ href_string); ("id", id_string) ]

let xml_of_ts_nte_ref (doc_settings : t_doc_settings) (nte_table : t_nte_table)
    (path : t_path) (a : ts_nte_ref) : Xml.xml =
  xml_of_string (string_of_ts_nte_ref doc_settings nte_table path a)

(* nte_inline *)

let attr_list_of_ts_nte_inline (doc_settings : t_doc_settings) (path : t_path)
    (a : ts_nte_inline) : (string * string) list =
  match a with
  | Cs_nte_inline (_, Cs_int i) ->
      let addendum : string = string_of_int i in
      let href_string : string = "NTE" ^ addendum in
      let id_string : string = "ref_" ^ href_string in
      [ ("href", "#" ^ href_string); ("id", id_string) ]

let xml_of_ts_nte_inline (doc_settings : t_doc_settings)
    (nte_table : t_nte_table) (path : t_path) (a : ts_nte_inline) : Xml.xml =
...
let xml_of_ts_txt_unit_wysiwyg (a : ts_txt_unit_wysiwyg) : Xml.xml =
  ...
  | Cs_txt_unit_wysiwyg (b : string) ->
      Xml.Element ("txt_unit_wysiwyg", [], [ xml_of_string b ])

let xml_of_ts_txt_unit_emph (a : ts_txt_unit_emph) : Xml.xml =
  match a with
  | Cs_txt_unit_emph (b : string) ->
      Xml.Element ("txt_unit_emph", [], [ xml_of_string b ])

let xml_of_ts_txt_unit_c_ref (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (path : t_path) (a : ts_txt_unit_c_ref) :
    Xml.xml =
  match a with
  | Cs_txt_unit_c_ref (b : ts_c_ref) ->
      Xml.Element
        ( "txt_unit_c_ref",
          attr_list_of_ts_c_ref doc_settings path b,
          [ xml_of_ts_c_ref doc_settings cref_table path b ] )

let xml_of_ts_txt_unit_nte_ref (doc_settings : t_doc_settings)
    (nte_table : t_nte_table) (path : t_path) (a : ts_txt_unit_nte_ref) :
    Xml.xml =
  match a with
  | Cs_txt_unit_nte_ref (b : ts_nte_ref) ->
      Xml.Element
        ( "txt_unit_nte",
          attr_list_of_ts_nte_ref doc_settings path b,
          [ xml_of_ts_nte_ref doc_settings nte_table path b ] )

let xml_of_ts_txt_unit_nte_inline (doc_settings : t_doc_settings)
    (nte_table : t_nte_table) (path : t_path) (a : ts_txt_unit_nte_inline) :
    Xml.xml =
  match a with
  | Cs_txt_unit_nte_inline (b : ts_nte_inline) ->
      Xml.Element
        ( "txt_unit_nte",
          attr_list_of_ts_nte_inline doc_settings path b,
          [ xml_of_ts_nte_inline doc_settings nte_table path b ] )

let xml_of_tu_txt_unit (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : tu_txt_unit) : Xml.xml =
  match a with
  | Cu_txt_unit_wysiwyg (b : ts_txt_unit_wysiwyg) ->
      xml_of_ts_txt_unit_wysiwyg b
  | Cu_txt_unit_emph (b : ts_txt_unit_emph) -> xml_of_ts_txt_unit_emph b
  | Cu_txt_unit_c_ref (b : ts_txt_unit_c_ref) ->
      xml_of_ts_txt_unit_c_ref doc_settings cref_table path b
  | Cu_txt_unit_nte_ref (b : ts_txt_unit_nte_ref) ->
      xml_of_ts_txt_unit_nte_ref doc_settings nte_table path b
  | Cu_txt_unit_nte_inline (b : ts_txt_unit_nte_inline) ->
      xml_of_ts_txt_unit_nte_inline doc_settings nte_table path b

let xml_list_of_ts_txt_units (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : ts_txt_units) : Xml.xml list =
  match a with
  | Cs_txt_units (b : tu_txt_unit list) ->
      List.map (xml_of_tu_txt_unit doc_settings cref_table nte_table path) b

let xml_list_of_ts_txt_lines (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (txt_lines : ts_txt_lines) : Xml.xml list =
...
let xml_of_ts_blk_txt (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (blk_txt : ts_blk_txt) : Xml.xml =
  match blk_txt with
...
let xml_of_tr_dsp_line (doc_settings : t_doc_settings)
    (cref_table : t_cref_table) (nte_table : t_nte_table) (path : t_path)
    (a : tr_dsp_line) : Xml.xml =
...
let xml_list_of_ts_vrb_lines (vrb_lines : ts_vrb_lines) : Xml.xml list =
  ...
  | Cs_vrb_lines (vrb_line_list : ts_vrb_line list) ->
      List.map xml_of_ts_vrb_line vrb_line_list

let xml_of_ts_blk_vrb (blk_vrb : ts_blk_vrb) : Xml.xml =
  match blk_vrb with
  | Cs_blk_vrb (vrb_lines : ts_vrb_lines) ->
      Xml.Element ("blk_vrb", [], xml_list_of_ts_vrb_lines vrb_lines)
...
let xml_of_ts_qtn_unit_wysiwyg (qtn_unit_wysiwyg : ts_qtn_unit_wysiwyg) :
    Xml.xml =
  match qtn_unit_wysiwyg with
  | Cs_qtn_unit_wysiwyg s ->
      Xml.Element ("txt_unit_wysiwyg", [], [ xml_of_string s ])

let xml_of_tu_qtn_unit (qtn_unit : tu_qtn_unit) : Xml.xml =
  match qtn_unit with
  | Cu_qtn_unit_wysiwyg qtn_unit_wysiwyg ->
      xml_of_ts_qtn_unit_wysiwyg qtn_unit_wysiwyg
  | Cu_qtn_unit_emph qtn_unit_emph -> xml_of_ts_qtn_unit_emph qtn_unit_emph

let xml_list_of_tu_qtn_unit_list (qtn_unit_list : tu_qtn_unit list) :
    Xml.xml list =
...
let xml_list_of_tu_qtn_line_list (qtn_line_list : tu_qtn_line list) :
    Xml.xml list =
  let space : Xml.xml =
  ...
  let rec aux (lst : tu_qtn_line list) (acc : Xml.xml list) =
    ...
    | lst_hd :: lst_tl -> (
        ...
        | _ :: _, Cu_qtn_line_std qtn_line_std ->
            aux lst_tl
              (List.concat
                 [ acc; [ space ]; xml_list_of_ts_qtn_line_std qtn_line_std ])
        | [], Cu_qtn_line_std qtn_line_std ->
            aux lst_tl
              (List.concat [ acc; xml_list_of_ts_qtn_line_std qtn_line_std ])
        | _, Cu_qtn_line_br qtn_line_br ->
            aux lst_tl
              (List.concat [ acc; br; xml_list_of_ts_qtn_line_br qtn_line_br ]))
  in
  aux qtn_line_list []
...
let xml_of_ts_blk_qtn (blk_qtn : ts_blk_qtn) : Xml.xml =
  ...
  | Cs_blk_qtn (qtn_lines : ts_qtn_lines) ->
      Xml.Element ("blk_qtn", [], xml_list_of_ts_qtn_lines qtn_lines)
...
let unite_exml_txt_units_wysiwyg (xml_list : Xml.xml list) : Xml.xml =
  let rec aux (lst : Xml.xml list) (acc : string) : string =
    ...
    | [] -> acc
    | hd :: tl -> (
        match hd with Xml.PCData s -> aux tl (acc ^ s) | _ -> aux tl acc)
  in
...
let normalize_exml_txt_units (xml_list : Xml.xml list) : Xml.xml list =
  let rec aux (lst : Xml.xml list) (acc_list : Xml.xml list)
    ...
    | [] -> (
        ...
        | [] -> acc_list
        | _ :: _ ->
            unite_exml_txt_units_wysiwyg (List.rev acc_wysiwyg) :: acc_list)
    | hd :: tl -> (
        match (hd, acc_wysiwyg) with
        | Xml.Element ("txt_unit_wysiwyg", _, [ xml ]), _ ->
            aux tl acc_list (xml :: acc_wysiwyg)
        | _, _ :: _ ->
            aux tl
              (hd
              :: unite_exml_txt_units_wysiwyg (List.rev acc_wysiwyg)
              :: acc_list)
              []
...
let rec normalize_exml (xml : Xml.xml) : Xml.xml =
  ...
  | Xml.Element (tag, attr_list, xml_list) -> (
      match tag with
      | "blk_qtn" | "blk_txt" | "dsp_line_main" | "par_hdr_inline" | "par_hdr"
      | "sec_hdr" | "ch_hdr" ->
          Xml.Element (tag, attr_list, normalize_exml_txt_units xml_list)
