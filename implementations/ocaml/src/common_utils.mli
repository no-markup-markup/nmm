(** A toolkit used by {!module:Compiler_of_doc} for handling default and custom
    document settings (margins, document width, etc.), cross-references, and
    labels. *)
...
val string_of_ts_c_ref :
  t_doc_settings -> t_cref_table -> t_path -> Doc_types.ts_c_ref -> string
(** [string_of_ts_c_ref doc_settings cref_table path c_ref] attempts to match
    [c_ref] ocurring at [path] with an [id] in [cref_table], and return a string
    representation of the path to [id] relative to the closest common ancestor
    of [c_ref] and [id].
...
val node_of_blk_itm :
  t_doc_settings -> t_path -> int -> Doc_types.tr_blk_itm -> t_node

val node_of_dsp_line :
  t_doc_settings -> t_path -> int -> Doc_types.tr_dsp_line -> t_node
...
val time_of_ts_date_auto :
  t_doc_settings -> Doc_types.ts_date_auto -> t_time option

(** {2 Footnotes} *)

type t_nte_entry =
  | Ftn_entry_ref of
      (Doc_types.ts_nte_ref * t_path * int * Doc_types.tr_blk_nte)
  | Ftn_entry_inline of (Doc_types.ts_nte_inline * t_path * int)
...
val string_of_ts_nte_ref :
  t_doc_settings -> t_nte_table -> t_path -> Doc_types.ts_nte_ref -> string

val string_of_ts_nte_inline :
  t_doc_settings -> t_nte_table -> t_path -> Doc_types.ts_nte_inline -> string
...
val reference_of_ts_nte_ref :
  ...
  t_cref_table ->
  t_path ->
  Doc_types.ts_nte_ref ->
  Doc_types.tr_blk_nte option
...
val txt_units_of_txt_lines :
  Doc_types.ts_txt_lines -> Doc_types.tu_txt_unit list
