...
let validate_exml (path : string) (exml : Xml.xml) : unit =
  ...
  with Dtd.Prove_error e ->
    raise
      (Error
         (String.concat " "
            [ path; "exml"; "->"; "Ddt.prove_error:"; Dtd.prove_error e ]))
...
let validate_axml (path : string) (axml : Xml.xml) : unit =
  ...
  with Dtd.Prove_error e ->
    raise
      (Error
         (String.concat " "
            [ path; "axml"; "->"; "Ddt.prove_error:"; Dtd.prove_error e ]))

let rec test_with_nmm_file (options : Common_utils.t_html_options)
    (path : string) : unit =
  try
    let doc : Doc_types.tr_doc =
      Main.doc_of_nmm (Common_utils.axml_options_of_html_options options) path
    in
...
and test_with_axml_file (options : Common_utils.t_html_options) (path : string)
    : unit =
  try
...
and identity_test_w_doc (path : string) (doc : Doc_types.tr_doc)
  ...
  | false ->
      raise
        (Error
           (String.concat " "
              [ path; "->"; "doc_of_axml (axml_of_doc doc) NOT EQUAL TO doc" ]))

and identity_test_w_axml (path : string) (axml : Xml.xml)
  ...
  | false ->
      raise
        (Error
           (String.concat " "
              [ path; "->"; "axml_of_doc (doc_of_axml axml) NOT EQUAL TO axml" ]))

and xml_right_test (path : string) (format : string) (doc : Doc_types.tr_doc)
    (xml : Xml.xml) : unit =
  try
          ...
          (Error
             (String.concat " "
                [
                  ...
                  "->";
                  format;
                  "NOT EQUAL TO Xml_right.parse_string (Xml_right.to_string";
                  format;
                  ")";
                ]))
  with Error s | Xml_right.Error s -> raise (Error s)

and xml_right_test_fmt (path : string) (format : string)
    (doc : Doc_types.tr_doc) (xml : Xml.xml) : unit =
  try
          ...
          (Error
             (String.concat " "
                [
                  ...
                  "->";
                  format;
                  "NOT EQUAL TO Xml_right.parse_string (Xml_right.to_string_fmt";
                  format;
                  ")";
                ]))
  with Error s | Xml_right.Error s -> raise (Error s)
