...
make_subset_font_css_main () {
	...
	local ascii_unicodes=$(printf '%04x ' $(seq 0 127))
	local unsorted_unicodes=$(iconv -f utf8 -t utf32 ${input_nmm_file} | hexdump -v -e '"%04x "')
	local unicodes=$(echo "${ascii_unicodes} ${unsorted_unicodes}" | sort -u -t ' ')
